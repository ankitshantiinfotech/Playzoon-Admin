...
import {
  ...
  ArrowLeft, Send, Building2, Globe, MapPin,
  FileText, Phone, Mail, User, Briefcase, Trophy,
} from "lucide-react";
...
import {
  getProviderWithHistory, getRejectedProviderIds,
  type ServiceProvider, type FieldSnapshot,
} from "../provider-data";
...
function validateForm(data: FieldSnapshot): { valid: boolean; errors: FieldError } {
  ...
  if (!data.sportCategory.trim()) errors.sportCategory = "Sport category is required";
...
const SPORT_OPTIONS = [
  ...
  "Running", "Boxing", "Volleyball", "Cricket", "Table Tennis",
];
...
export function ProviderPortalPage() {
  ...
  // ── Form handlers ──────────────────────────────────────
  const updateField = useCallback((field: keyof FieldSnapshot, value: string) => {
    setFormData(prev => prev ? { ...prev, [field]: value } : prev);
    setTouched(prev => new Set(prev).add(field));
  }, []);

  const handleBlur = useCallback((field: keyof FieldSnapshot) => {
    setTouched(prev => new Set(prev).add(field));
    if (formData) {
  ...
  const handleReSubmit = useCallback(async () => {
    ...
    try {
      ...
      toast.success("Your application has been re-submitted for review.", { duration: 5000 });
    } catch {
      toast.error("Failed to re-submit application. Please try again.", { duration: 8000 });
    } finally {
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-center min-h-[50vh]">
          <div className="text-center space-y-5 max-w-lg">
            ...
            <h1 className="text-2xl text-[#111827]">Application Re-Submitted</h1>
            <p className="text-sm text-[#6B7280] leading-relaxed">
              Your application has been re-submitted for review. You will be notified once your application has been reviewed by our team.
            </p>
            <div className="flex items-center justify-center gap-2 pt-2">
              ...
              {(provider.resubmissionCount || 0) > 0 && (
                <Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-200">
                  Re-submitted {provider.resubmissionCount} {provider.resubmissionCount === 1 ? "time" : "times"}
                </Badge>
              )}
            </div>
            <Button
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-[#6B7280] mt-0.5">
              Viewing as: <span className="text-[#111827]">{provider.name}</span>
            </p>
          </div>
        </div>
        ...
        >
          ...
          <div className="flex-1 space-y-1">
            ...
            <p className="text-xs text-red-500 mt-2">
              Please review and correct your information below, then re-submit your application.
            </p>
          </div>
        </div>
      )}
        ...
        >
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-amber-600 mt-1">
              Your application is currently being reviewed. You cannot make changes until the review is complete.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="bg-white border rounded-xl overflow-hidden">
        <div className="px-5 py-4 border-b bg-gray-50/50">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              {isRejected && (
                <Badge variant="outline" className="bg-red-100 text-red-800 border-red-200 text-[11px]">
                  Rejected
                </Badge>
              )}
              {isPending && (
                <Badge variant="outline" className="bg-amber-100 text-amber-800 border-amber-200 text-[11px]">
                  Pending Review
                </Badge>
              )}
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <FormField
                label="Provider Name"
                required
                error={touched.has("providerName") ? errors.providerName : undefined}
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <FormField
                ...
                required
                error={touched.has("licenseNumber") ? errors.licenseNumber : undefined}
              ...
              <FormField
                ...
                required
                error={touched.has("sportCategory") ? errors.sportCategory : undefined}
        ...
        {isRejected && (
          <div className="px-5 py-4 border-t bg-gray-50/50 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <p className="text-xs text-[#9CA3AF]">
              Review your details carefully, then click Re-Submit to send your updated application for review.
            </p>
            <Button
              ref={resubmitBtnRef}
          ...
          <div className="px-5 py-4 border-b bg-gray-50/50">
            <div className="flex items-center justify-between">
              ...
              <Badge variant="outline" className="bg-gray-100 text-gray-600 border-gray-200 text-[10px]">
                {provider.submissionHistory.length} {provider.submissionHistory.length === 1 ? "submission" : "submissions"}
              </Badge>
            </div>
          </div>
          <div className="p-5">
                  ...
                  <div key={entry.submissionNumber} className="flex gap-4">
                    ...
                    <div className={cn("pb-6 flex-1 -mt-1", isLast && "pb-0")}>
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <Badge
                          ...
                          className={cn(
                            ...
                            entry.status === "Rejected" && "bg-red-100 text-red-800 border-red-200",
                          )}
                        >
                          {entry.status}
                        </Badge>
                      ...
                      <p className="text-xs text-[#9CA3AF] mt-0.5">
                        Submitted {format(entry.submittedAt, "MMM dd, yyyy 'at' HH:mm")}
                        {entry.reviewedAt && (
                          <> &middot; Reviewed {format(entry.reviewedAt, "MMM dd, yyyy")}</>
                        )}
                      </p>
      ...
      >
        <DialogContent className="sm:max-w-[440px]">
          <DialogHeader>
            ...
            <DialogTitle className="text-center">Confirm Re-Submission</DialogTitle>
            <DialogDescription className="text-center">
              Are you sure you want to re-submit your application with updated details?
            </DialogDescription>
          </DialogHeader>
