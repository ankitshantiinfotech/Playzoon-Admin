...
import {
  ArrowLeft, Upload, X, Camera, RotateCw, RotateCcw,
  ZoomIn, ZoomOut, CheckCircle2, XCircle, Loader2,
  CalendarIcon, Search, ChevronDown, User, Save,
  ShieldCheck, Lock, Unlock, GraduationCap,
} from "lucide-react";
...
import {
  COUNTRY_CODES,
  GENDERS, NATIONALITIES, LANGUAGES,
  INITIAL_COACH_FORM,
  ACCEPTED_PHOTO_TYPES, MAX_PHOTO_SIZE, formatFileSize,
  checkEmailDuplicate,
  validateFirstName, validateLastName, validateDOB,
  validateGender, validateEmail, validateMobileNumber,
  validateNationality, getCoachDetail,
  type CoachFormState, type CoachFieldErrors, type CoachTouchedFields,
  type Gender,
} from "./coach-form-data";
import {
  CoachMediaSection, INITIAL_MEDIA_STATE,
  type CoachMediaState,
} from "./CoachMediaSection";
import {
  CoachAvailabilitySection, createInitialAvailability,
  type CoachAvailabilityState,
} from "./CoachAvailabilitySection";
import {
  CoachProfessionalSection, createInitialProfessional,
  getMockProfessional, validateProfessional,
  type CoachProfessionalState, type ProfessionalErrors,
  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        className={cn(
          "flex items-center gap-1.5 h-9 px-2.5 border rounded-md text-sm bg-white hover:bg-gray-50 transition-colors min-w-[110px]",
  ...
  return (
    <div ref={ref} className="relative space-y-1.5">
      ...
      <button
        ...
        onClick={() => setOpen(!open)}
        className={cn(
          "flex items-center justify-between w-full h-9 px-3 border rounded-md text-sm bg-white hover:bg-gray-50 transition-colors text-left",
          ...
          <div className="overflow-y-auto max-h-44">
            ...
            {filtered.length === 0 && (
              <p className="text-xs text-gray-400 text-center py-4">
                {values.length === options.length ? "All languages selected" : "No results found"}
              </p>
            )}
          </div>
        </div>
...
function ImageCropperModal({
  ...
  const [state, setState] = useState<CropperState>({ zoom: 1, rotation: 0, panX: 0, panY: 0 });
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    setState(s => ({ ...s, panX: dragStart.current.panX + dx, panY: dragStart.current.panY + dy }));
  };
  ...
  const handleTouchStart = (e: React.TouchEvent) => {
    ...
    isDragging.current = true;
    const t = e.touches[0];
    dragStart.current = { x: t.clientX, y: t.clientY, panX: state.panX, panY: state.panY };
  };
  ...
  const handleCrop = () => {
    ...
    ctx.translate(outSize / 2 + state.panX * scaleFactor, outSize / 2 + state.panY * scaleFactor);
    ctx.rotate((state.rotation * Math.PI) / 180);
    ctx.scale(state.zoom, state.zoom);
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <Slider
                ...
                onValueChange={([v]) => setState(s => ({ ...s, zoom: v / 100 }))}
                className="flex-1"
              />
              <ZoomIn className="h-4 w-4 text-gray-400 shrink-0" />
              <span className="text-xs text-gray-500 w-10 text-right">{Math.round(state.zoom * 100)}%</span>
            </div>
            ...
            <div className="flex items-center justify-center gap-2">
              ...
              <span className="text-xs text-gray-500 min-w-[50px] text-center">{state.rotation}deg</span>
              <Button
                type="button"
                variant="outline"
                size="sm"
...
export function FreelancerCoachFormPage() {
  ...
  const [availability, setAvailability] = useState<CoachAvailabilityState>(createInitialAvailability());
  ...
  // ── Field updater ─────────────────────────────────────
  const updateField = useCallback(<K extends keyof CoachFormState>(
    field: K,
  ...
  const validateField = useCallback((field: keyof CoachFormState, value: unknown): string | null => {
    switch (field) {
      case "firstName": return validateFirstName(value as string);
      case "lastName": return validateLastName(value as string);
      case "dob": return validateDOB(value as Date | null);
      case "gender": return validateGender(value as string);
      case "email": return validateEmail(value as string);
      case "mobileNumber": return validateMobileNumber(value as string);
      case "nationality": return validateNationality(value as string);
      default: return null;
    }
  }, []);

  // ── Validate on blur ──────────────────────────────────
  const handleBlur = useCallback((field: keyof CoachFormState) => {
    touchField(field);
  ...
  useEffect(() => {
    ...
    if (isEditMode && trimmed.toLowerCase() === initialEmailRef.current.toLowerCase()) {
  ...
  const validateAll = useCallback((): boolean => {
    const fields: (keyof CoachFormState)[] = [
      ...
      "email", "mobileNumber", "nationality",
    ];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!validateAll()) {
      ...
      const firstErrorSection = Object.keys(errors).length > 0
        ? "sec-personal"
      ...
      if (firstErrorSection) {
        setTimeout(() => {
          document.getElementById(firstErrorSection)?.scrollIntoView({ behavior: "smooth", block: "start" });
        }, 100);
      }
      return;
    }
    ...
    setIsSaving(false);
    setIsDirty(false);
    toast.success(isEditMode ? "Coach updated successfully" : "Coach created successfully");
    ...
    <form onSubmit={handleSubmit} className="space-y-6 max-w-3xl mx-auto pb-12">
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        >
          {isSaving ? (
            ...
            <><Save className="h-3.5 w-3.5" /> {isEditMode ? "Update Coach" : "Create Coach"}</>
          )}
        </Button>
      </div>
      ...
      <div className="sticky top-0 z-40 -mx-1 px-1">
        <div className="bg-white/95 backdrop-blur-sm border rounded-lg px-2 py-1.5 flex items-center gap-1 overflow-x-auto scrollbar-hide shadow-sm">
          {[
            { id: "sec-personal", label: "Personal", icon: <User className="h-3 w-3" />, hasError: Object.keys(errors).length > 0 },
            { id: "sec-professional", label: "Professional", icon: <GraduationCap className="h-3 w-3" />, hasError: Object.keys(professionalErrors).length > 0 },
            { id: "sec-status", label: "Status", icon: <ShieldCheck className="h-3 w-3" />, hasError: false },
            { id: "sec-media", label: "Media", icon: <Upload className="h-3 w-3" />, hasError: false },
            { id: "sec-availability", label: "Availability", icon: <CalendarIcon className="h-3 w-3" />, hasError: false },
          ].map(sec => (
            <button
              key={sec.id}
              type="button"
      ...
      {/* ═══════════════════════════════════════════════════
          ...
          ════════════���══════════════════════════════════════ */}
      <div id="sec-personal" className="bg-white border rounded-xl overflow-hidden scroll-mt-16">
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-[10px] text-[#9CA3AF]">JPG or PNG, max 5MB. 1:1 aspect ratio recommended.</p>
            <input
              ref={fileInputRef}
              type="file"
              accept=".jpg,.jpeg,.png"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                className={cn(
                  "h-9",
                  touched.firstName && errors.firstName && "border-red-300 ring-1 ring-red-200"
                )}
              />
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                className={cn(
                  "h-9",
                  touched.lastName && errors.lastName && "border-red-300 ring-1 ring-red-200"
                )}
              />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <PopoverTrigger asChild>
                  <Button
                    ...
                    className={cn(
                      ...
                      touched.dob && errors.dob && "border-red-300 ring-1 ring-red-200"
                    )}
                  >
                    <CalendarIcon className="h-3.5 w-3.5 mr-2 text-gray-400" />
                    {form.dob ? format(form.dob, "dd/MM/yyyy") : "Select date of birth"}
                  </Button>
                </PopoverTrigger>
                <PopoverContent className="w-auto p-0" align="start">
                  <Calendar
                    ...
                    onSelect={date => {
                      ...
                      setErrors(p => err ? { ...p, dob: err } : (() => { const n = { ...p }; delete n.dob; return n; })());
                    }}
            ...
            <div className="space-y-1.5">
              ...
              <RadioGroup
                ...
                className="flex items-center gap-4 h-9 pt-1"
              >
                {GENDERS.map(g => (
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              <Input
                ...
                className={cn(
                  "h-9 pr-9",
                  touched.email && errors.email && "border-red-300 ring-1 ring-red-200",
                  emailCheckState === "unique" && "border-emerald-300 ring-1 ring-emerald-200",
                )}
              />
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex gap-2">
              ...
              <Input
                ...
                className={cn(
                  "h-9 flex-1",
                  touched.mobileNumber && errors.mobileNumber && "border-red-300 ring-1 ring-red-200"
                )}
              />
            </div>
      ...
      {/* ═══════════════════════════════════════════════════
          ...
          ═══════════════════════════════════════════════════ */}
      <div id="sec-status" className="bg-white border rounded-xl overflow-hidden scroll-mt-16">
        <div className="px-6 py-4 border-b bg-gray-50/50">
          <h2 className="text-sm text-[#111827] flex items-center gap-2">
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="flex items-center justify-between gap-4 py-3 px-4 rounded-lg border bg-gray-50/50">
            <div className="flex items-center gap-3 min-w-0">
              <div className={cn(
                ...
                form.accountStatus === "Locked" ? "bg-red-100" : "bg-emerald-100",
              )}>
            ...
            <div className="flex items-center gap-2 shrink-0">
              <span className={cn(
                "text-xs",
                form.accountStatus === "Locked" ? "text-red-600" : "text-emerald-600",
              )}>
                {form.accountStatus}
              </span>
              <Switch
          ...
          <div className="flex items-center justify-between gap-4 py-3 px-4 rounded-lg border bg-gray-50/50">
            <div className="flex items-center gap-3 min-w-0">
              <div className={cn(
                "flex items-center justify-center h-8 w-8 rounded-lg shrink-0",
                form.platformStatus === "Active" ? "bg-emerald-100" : "bg-gray-100",
              )}>
                <span className={cn(
            ...
            <div className="flex items-center gap-2 shrink-0">
              <span className={cn(
                "text-xs",
                form.platformStatus === "Active" ? "text-emerald-600" : "text-gray-500",
              )}>
                {form.platformStatus}
              </span>
              <Switch
                ...
                onCheckedChange={(checked) => {
                  updateField("platformStatus", checked ? "Active" : "Inactive");
                  toast.success(checked ? "Coach is now active on the platform." : "Coach is now inactive.");
                }}
      ...
      <ImageCropper
        open={cropperOpen}
        onOpenChange={(open) => { if (!open) { setCropperOpen(false); setRawImageSrc(""); } }}
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to lock this coach's account? They will be unable to log in
