...
import {
  ...
  ACCEPTED_PHOTO_TYPES, MAX_DOC_SIZE, MAX_PHOTO_SIZE,
  type UploadedFile,
  checkEmailDuplicate,
  validateClubName, validateName, validateEmail,
  validateMobileNumber, validateLandline, validateDateOfIncorporation,
  formatFileSize,
...
export function TrainingProviderFormPage() {
  ...
  useEffect(() => {
    ...
    const dirty = isEditMode
      ...
      : Object.values(form).some(v => (typeof v === "string" ? v.trim() !== "" : v !== null)) || documents.length > 0;
    setIsDirty(dirty);
  ...
  const validateField = useCallback((field: keyof FormState, value: unknown): string | null => {
    switch (field) {
      case "clubName": return validateClubName(value as string);
      case "firstName": return validateName(value as string, "First Name");
      case "lastName": return validateName(value as string, "Last Name");
      ...
      case "dateOfIncorporation": return validateDateOfIncorporation(value as Date | null);
      default: return null;
    }
  }, []);
  ...
  const validateAll = useCallback((): boolean => {
    const fields: (keyof FormState)[] = [
      "clubName", "firstName", "lastName", "email",
      "mobileNumber", "dateOfIncorporation", "landline",
    ];
  ...
  const isFormValid = useMemo(() => {
    const requiredFields: (keyof FormState)[] = [
      "clubName", "firstName", "lastName", "email", "mobileNumber", "dateOfIncorporation",
    ];
    ...
    const docsOk = isEditMode || documents.filter(d => d.status === "complete").length >= 1;
    const allUploaded = documents.every(d => d.status !== "uploading");

    return allFilled && noErrors && emailOk && docsOk && allUploaded && !submitting;
  ...
  const processDocumentFiles = useCallback((files: FileList | File[]) => {
    ...
    fileArray.forEach(file => {
      ...
      if (!ACCEPTED_DOC_TYPES.includes(file.type)) {
        toast.error("Unsupported file format. Accepted: PDF, PNG, JPEG, DOC, XLS.");
        return;
      }
      // Validate size
  ...
  const handlePhotoCropComplete = useCallback((_blob: Blob, previewUrl: string) => {
    ...
    const interval = setInterval(() => {
      ...
      if (progress >= 100) {
        ...
        setProfilePhoto(prev =>
          prev ? { ...prev, progress: 100, status: "complete" as const } : null
        );
      } else {
        setProfilePhoto(prev =>
  ...
  if (notFound) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <p className="text-sm text-gray-500 mb-4">The training provider you're looking for does not exist.</p>
        ...
        <div className="bg-white rounded-xl border border-gray-200 divide-y divide-gray-100">
          ...
          <div className="p-6">
            <h2 className="text-sm text-gray-500 mb-4 flex items-center gap-2">
              ...
              <Badge variant="outline" className="text-[10px] text-gray-400 border-gray-200 ml-1">Optional</Badge>
            </h2>
            ...
            <div className="flex items-start gap-6">
              ...
              <div className="space-y-2">
                <input
                  ...
                  className="hidden"
                  onChange={e => {
                    if (e.target.files?.[0]) processProfilePhoto(e.target.files[0]);
                    e.target.value = "";
                  }}
                />
          ...
          <div className="p-6 space-y-5">
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                className={cn(
                  "h-9 text-sm",
                  touched.clubName && errors.clubName && "border-red-400 focus-visible:ring-red-200"
                )}
                aria-invalid={!!(touched.clubName && errors.clubName)}
                aria-describedby={touched.clubName && errors.clubName ? "clubName-error" : undefined}
              />
              <div className="flex justify-between">
                {touched.clubName && errors.clubName ? (
                  <p id="clubName-error" className="text-[11px] text-red-500">{errors.clubName}</p>
                ) : <span />}
                <span className="text-[11px] text-gray-400">{form.clubName.length}/200</span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  className={cn(
                    "h-9 text-sm",
                    touched.firstName && errors.firstName && "border-red-400 focus-visible:ring-red-200"
                  )}
                  aria-invalid={!!(touched.firstName && errors.firstName)}
                  aria-describedby={touched.firstName && errors.firstName ? "firstName-error" : undefined}
                />
                {touched.firstName && errors.firstName && (
                  <p id="firstName-error" className="text-[11px] text-red-500">{errors.firstName}</p>
                )}
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  className={cn(
                    "h-9 text-sm",
                    touched.lastName && errors.lastName && "border-red-400 focus-visible:ring-red-200"
                  )}
                  aria-invalid={!!(touched.lastName && errors.lastName)}
                  aria-describedby={touched.lastName && errors.lastName ? "lastName-error" : undefined}
                />
                {touched.lastName && errors.lastName && (
                  <p id="lastName-error" className="text-[11px] text-red-500">{errors.lastName}</p>
                )}
              </div>
            </div>
            ...
            <div className="space-y-1.5">
                ...
                <PopoverTrigger asChild>
                  <Button
                    ...
                    className={cn(
                      ...
                      touched.dateOfIncorporation && errors.dateOfIncorporation && "border-red-400"
                    )}
                ...
                <PopoverContent className="w-auto p-0" align="start">
                  <Calendar
                    ...
                    onSelect={(date) => {
                      ...
                      setTouched(prev => ({ ...prev, dateOfIncorporation: true }));
                      setDatePickerOpen(false);
                      // Validate immediately
                      const err = validateDateOfIncorporation(date ?? null);
                      setErrors(prev => ({ ...prev, dateOfIncorporation: err ?? undefined }));
                    }}
              ...
              {touched.dateOfIncorporation && errors.dateOfIncorporation && (
                <p className="text-[11px] text-red-500">{errors.dateOfIncorporation}</p>
              )}
            </div>
