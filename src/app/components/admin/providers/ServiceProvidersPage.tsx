...
import {
  ...
  Building2, GraduationCap, ShieldX,
  CheckCircle2, XCircle, Info, AlertTriangle, Loader2,
  Check, Ban, Plus, Filter,
} from "lucide-react";
...
import { BulkActionBar, type BulkActionType, type BulkActionResult } from "./components/BulkActionBar";
...
type StatusFilter = "all" | "active" | "inactive" | "verified" | "unverified" | "locked";
...
const STATUS_FILTER_OPTIONS: StatusFilterOption[] = [
  { key: "all",        label: "All",        activeBg: "bg-[#003B95]", activeText: "text-white",       activeBorder: "border-[#003B95]", dot: "" },
  { key: "active",     label: "Active",     activeBg: "bg-emerald-50", activeText: "text-emerald-700", activeBorder: "border-emerald-300", dot: "bg-emerald-500" },
  { key: "inactive",   label: "Inactive",   activeBg: "bg-gray-100",   activeText: "text-gray-700",   activeBorder: "border-gray-300",   dot: "bg-gray-400" },
  { key: "verified",   label: "Verified",   activeBg: "bg-emerald-50", activeText: "text-emerald-700", activeBorder: "border-emerald-300", dot: "bg-emerald-500" },
  { key: "unverified", label: "Unverified", activeBg: "bg-amber-50",   activeText: "text-amber-700",  activeBorder: "border-amber-300",  dot: "bg-amber-500" },
  { key: "locked",     label: "Locked",     activeBg: "bg-red-50",     activeText: "text-red-700",    activeBorder: "border-red-300",    dot: "bg-red-500" },
];

/** Apply status filter predicate to a provider-like object */
function matchesStatusFilter<T extends { platformStatus: string; verificationStatus: string; accountStatus: string }>(
  item: T,
  filter: StatusFilter,
): boolean {
  switch (filter) {
    case "all":        return true;
    case "active":     return item.platformStatus === "Active";
    case "inactive":   return item.platformStatus === "Inactive";
    case "verified":   return item.verificationStatus === "Approved";
    case "unverified": return item.verificationStatus === "Pending";
    case "locked":     return item.accountStatus === "Locked";
  }
}
...
function TruncatedCell({ text, maxLength, className }: { text: string; maxLength: number; className?: string }) {
...
export function ServiceProvidersPage() {
  ...
  const [facilitySortField, setFacilitySortField] = useState<FacilitySortField>("name");
  ...
  const [approveTarget, setApproveTarget] = useState<ServiceProvider | null>(null);
  ...
  useEffect(() => { setSelectedIds(new Set()); }, [activeTab, subTab, statusFilter, page]);
  ...
  const pendingCount = useMemo(() => {
    if (activeTab === "Onboarding") return providers.filter(p => p.verificationStatus === "Pending").length;
  ...
  const statusFilterCounts = useMemo(() => {
    const counts: Record<StatusFilter, number> = {
      all: 0, active: 0, inactive: 0, verified: 0, unverified: 0, locked: 0,
    };

    let base = activeTab === "Onboarding"
      ? providers.filter(p => p.verificationStatus === "Pending")
      : providers.filter(p => p.providerType === activeTab);
    if (activeTab !== "Onboarding" && subTab === "onboarding") base = base.filter(p => p.verificationStatus === "Pending");
  ...
  const filtered = useMemo(() => {
    let result = activeTab === "Onboarding"
      ? providers.filter(p => p.verificationStatus === "Pending")
      : providers.filter(p => p.providerType === activeTab);
      ...
      result.sort((a, b) => {
        ...
        switch (facilitySortField) {
          ...
          case "incorporationDate":
            cmp = (a.incorporationDate?.getTime() ?? 0) - (b.incorporationDate?.getTime() ?? 0);
            break;
          case "email":
            cmp = a.email.localeCompare(b.email);
            break;
          case "personInCharge":
            cmp = (a.personInCharge ?? "").localeCompare(b.personInCharge ?? "");
            break;
    ...
    return result;
  }, [providers, activeTab, subTab, debouncedSearch, sortField, sortDir, facilitySortField, facilitySortDir, statusFilter]);
  ...
  const activeTotalPages = Math.max(1, Math.ceil(activeFiltered.length / pageSize));
  ...
  const handleBulkAction = useCallback(async (
    ...
    let processed = 0, skipped = 0, skipReason = "";
    ...
    for (let i = 0; i < ids.length; i++) {
      ...
      if (p) {
        ...
        if (action === "reject" && p.verificationStatus === "Rejected") { shouldSkip = true; skipReason = "already rejected"; }
        if (action === "lock" && p.accountStatus === "Locked") { shouldSkip = true; skipReason = "already locked"; }
        if (action === "unlock" && p.accountStatus === "Unlocked") { shouldSkip = true; skipReason = "already unlocked"; }
        if (action === "activate" && p.platformStatus === "Active") { shouldSkip = true; skipReason = "already active"; }
        if (action === "deactivate" && p.platformStatus === "Inactive") { shouldSkip = true; skipReason = "already inactive"; }
      }
      ...
      if (shouldSkip) { skipped++; }
        ...
        setProviders(prev => prev.map(p => {
          if (p.id !== id) return p;
          switch (action) {
            case "approve": return { ...p, verificationStatus: "Approved" as const, platformStatus: "Active" as const };
            case "reject": return { ...p, verificationStatus: "Rejected" as const, rejectionReason };
            case "lock": return { ...p, accountStatus: "Locked" as const };
            case "unlock": return { ...p, accountStatus: "Unlocked" as const };
            case "activate": return { ...p, platformStatus: "Active" as const };
            case "deactivate": return { ...p, platformStatus: "Inactive" as const };
            default: return p;
          }
        }));
        processed++;
      ...
      setProviders(prev =>
        prev.map(p =>
          p.id === approveTarget.id
            ? { ...p, verificationStatus: "Approved" as const, platformStatus: "Active" as const }
            : p
        )
      );
      ...
      setProviders(prev =>
        prev.map(p =>
          p.id === rejectTarget.id
            ? { ...p, verificationStatus: "Rejected" as const, rejectionReason: reason }
            : p
        )
      );
  ...
  const getPageNumbers = (): number[] => {
    if (activeTotalPages <= 7) return Array.from({ length: activeTotalPages }, (_, i) => i + 1);
    const pages: number[] = [];
    if (page <= 4) {
      ...
      pages.push(1);
      pages.push(-1);
      for (let i = activeTotalPages - 4; i <= activeTotalPages; i++) pages.push(i);
    } else {
      pages.push(1);
      pages.push(-1);
    ...
    <div className="space-y-5">
      ...
      {banner.visible && (
        ...
        >
          ...
          <button onClick={() => setBanner(b => ({ ...b, visible: false }))} className="p-1 rounded hover:bg-black/5" aria-label="Dismiss banner">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h1 className="text-2xl text-[#111827] tracking-tight">Provider Management</h1>
          <p className="text-sm text-[#6B7280] mt-1">
            View and manage all registered service providers, approve onboarding requests, and monitor account statuses.
          </p>
        </div>
        {activeTab === "Training Provider" && (
          <Button
            className="bg-[#003B95] hover:bg-[#002d73] text-white text-sm h-9 shrink-0"
      ...
      <div className="bg-white border rounded-xl overflow-hidden">
        {/* Tab bar */}
        <div className="border-b" role="tablist" aria-label="Provider type tabs">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 px-5 py-3 border-b bg-gray-50/50">
          {/* Sub-tabs */}
          <div className="flex items-center gap-1" role="tablist" aria-label="Provider sub-tabs">
            <button
              role="tab"
            ...
            >
              Onboarding Requests
              {pendingCount > 0 && (
                <Badge variant="outline" className="text-[10px] h-4 min-w-[16px] justify-center bg-amber-100 text-amber-700 border-amber-200">
                  {pendingCount}
                </Badge>
              )}
            </button>
        ...
        <div id={`tabpanel-${activeTab}`} role="tabpanel" aria-label={`${activeTab} providers list`}>
          {isLoading ? (
            /* ── Skeleton Loader ──────────────────────── */
            ...
            <div className="text-center py-20 space-y-3">
              <div className="mx-auto w-16 h-16 rounded-full bg-gray-50 flex items-center justify-center">
                {statusFilter !== "all" ? (
                  ...
                  (() => { const Icon = TAB_ICONS[activeTab]; return <Icon className="h-8 w-8 text-gray-300" />; })()
                )}
              </div>
            ...
            <>
              ...
              <div className="overflow-x-auto">
                <Table>
                  <caption className="sr-only">{activeTab} providers list</caption>
                  <TableHeader>
                    <TableRow className="bg-gray-50/80 hover:bg-gray-50/80">
                      ...
                      <TableHead className="px-3 w-10">
                        <Checkbox
                          checked={
                            pagedProviders.length > 0 && pagedProviders.every(p => selectedIds.has(p.id))
                              ? true
                              : pagedProviders.some(p => selectedIds.has(p.id))
                                ? "indeterminate"
                                : false
                          }
                      ...
                      <SortableHeader field="email" className="hidden md:table-cell">Email ID</SortableHeader>
                      <TableHead className="px-4 hidden lg:table-cell">Mobile Number</TableHead>
                      {activeTab !== "Onboarding" && (
                        <SortableHeader field="verificationStatus">Verification</SortableHeader>
                      )}
                      <TableHead className="px-4 hidden md:table-cell">Account</TableHead>
                      <TableHead className="px-4 hidden lg:table-cell">Platform</TableHead>
                      <SortableHeader field="createdAt" className="hidden xl:table-cell">Created Date</SortableHeader>
                      <TableHead className="px-4 w-[140px]"><span className="sr-only">Actions</span></TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {pagedProviders.map(provider => (
                        ...
                        >
                          ...
                          {activeTab === "Onboarding" && (
                            <TableCell className="px-4">
                              <ProviderTypeBadge type={provider.onboardingType ?? provider.providerType} />
                            </TableCell>
                          )}
                            ...
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <span className="text-xs text-[#6B7280] cursor-default">
                                  {format(provider.createdAt, "dd/MM/yyyy HH:mm")}
                                </span>
                              </TooltipTrigger>
                              <TooltipContent>
                                {formatDistanceToNow(provider.createdAt, { addSuffix: true })}
                              </TooltipContent>
                            </Tooltip>
                          </TableCell>
                          ...
                          <TableCell className="px-4">
                            <div className="flex items-center gap-1">
                              ...
                              <Tooltip>
                                ...
                                <TooltipContent>
                                  {globalChatEnabled ? "Open Chat" : "Chat is disabled"}
                                </TooltipContent>
                              </Tooltip>
                              ...
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <Button
                                    ...
                                    className="h-7 w-7"
                                    onClick={() => {
                                      if (provider.providerType === "Training Provider") {
                                        navigate(`/providers/${provider.id}`);
                                      } else {
                                        navigate(`/provider-portal/${provider.id}`);
                                      }
                                    }}
                                  ...
                                  <Button
                                    ...
                                    onClick={() => {
                                      if (provider.providerType === "Training Provider") {
                                        ...
                                        toast.info(`Navigating to edit form for ${provider.name}...`);
                                      }
                                    }}
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between px-5 py-3 border-t bg-gray-50/50 gap-3">
                <div className="flex items-center gap-2 text-xs text-[#6B7280]">
                  ...
                  <span className="ml-2">
                    Showing {Math.min((page - 1) * pageSize + 1, activeFiltered.length)}–{Math.min(page * pageSize, activeFiltered.length)} of {activeFiltered.length}
                  </span>
                </div>
                ...
                <div className="flex items-center gap-1">
                  ...
                  {getPageNumbers().map((p, i) => {
                    if (p < 0) {
                      return <span key={`ellipsis-${i}`} className="px-1 text-xs text-gray-400">...</span>;
                    }
                    return (
                      <Button
                        key={p}
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to delete <span className="text-[#111827]">{deleteTarget?.name}</span>?
      ...
      <ApproveConfirmModal
        ...
        onCancel={() => { if (!isApproving) setApproveTarget(null); }}
      />
      ...
      <RejectReasonModal
        ...
        onCancel={() => { if (!isRejecting) setRejectTarget(null); }}
      />
    </div>
  );
}