...
import {
  Plus, Eye, Pencil, Trash2, UserPlus, Users,
  Mail, Phone, Award, Calendar, AlertTriangle,
  Search, X, Upload, ChevronDown, ChevronRight,
  Globe, Languages, Briefcase, GraduationCap, Tag,
  ImageIcon,
} from "lucide-react";
...
import type {
  ProviderCoach, CoachStatus, CoachGender, CoachQualification, CoachExperience,
} from "./training-provider-detail-data";

// ─── Constants ────────────────────────────────────────────────

const SPORTS_OPTIONS = [
  "Football", "Basketball", "Tennis", "Swimming", "Padel",
  "Volleyball", "Cricket", "Martial Arts", "Gymnastics", "Yoga",
  "CrossFit", "Cycling", "Boxing", "Track & Field", "Diving",
];

const NATIONALITY_OPTIONS = [
  "Saudi", "Emirati", "Bahraini", "Qatari", "Kuwaiti",
  "Egyptian", "Jordanian", "Lebanese", "Indian", "Pakistani",
  "British", "American", "Canadian", "Australian", "Filipino",
  "Sudanese", "Moroccan", "Tunisian", "Iraqi", "Omani",
];

const LANGUAGE_OPTIONS = [
  "English", "Arabic", "French", "Spanish", "Urdu",
  "Hindi", "Tagalog", "Bengali", "Portuguese", "German",
  "Mandarin", "Japanese", "Korean", "Turkish", "Persian",
];
...
const STATUS_COLORS: Record<CoachStatus, { bg: string; text: string; dot: string }> = {
...
function CoachStatusBadge({ status }: { status: CoachStatus }) {
  ...
  return (
    <Badge variant="outline" className={cn("text-[11px] border-0 gap-1.5", c.bg, c.text)}>
          ...
          <ScrollArea className="max-h-48">
            {options.map(opt => {
              ...
              return (
                ...
                >
                  {opt}
                  {selected && <span className="float-right text-[#003B95]">&#10003;</span>}
                </button>
              );
            })}
          </ScrollArea>
  ...
  return (
    <div className="border border-gray-200 rounded-lg">
      ...
      >
        ...
        <span className="text-sm font-medium text-[#111827] flex-1">{title}</span>
        {open
...
function CoachFormSheet({
  ...
  const update = <K extends keyof CoachFormData>(key: K, val: CoachFormData[K]) => {
  ...
  const addQualification = () => {
    const newQ: CoachQualification = { id: `Q-${Date.now()}`, title: "", description: "" };
    update("qualifications", [...form.qualifications, newQ]);
  };
  const updateQualification = (id: string, field: "title" | "description", val: string) => {
    update("qualifications", form.qualifications.map(q => q.id === id ? { ...q, [field]: val } : q));
  };
  const removeQualification = (id: string) => {
    update("qualifications", form.qualifications.filter(q => q.id !== id));
  };

  // ── Experience helpers ──
  const addExperience = () => {
    const newE: CoachExperience = { id: `E-${Date.now()}`, title: "", description: "", startDate: new Date(), endDate: new Date() };
    update("experiences", [...form.experiences, newE]);
  };
  const updateExperience = (id: string, field: keyof CoachExperience, val: string | Date) => {
    update("experiences", form.experiences.map(e => e.id === id ? { ...e, [field]: val } : e));
  };
  const removeExperience = (id: string) => {
    update("experiences", form.experiences.filter(e => e.id !== id));
  };
      ...
      <SheetContent side="right" className="sm:max-w-2xl w-full p-0 flex flex-col">
        <SheetHeader className="px-6 pt-6 pb-4 border-b border-gray-100 shrink-0">
          <SheetTitle className="flex items-center gap-2 text-lg">
            {mode === "add" ? <UserPlus className="h-5 w-5 text-[#003B95]" /> : <Pencil className="h-5 w-5 text-[#003B95]" />}
        ...
        <ScrollArea className="flex-1 overflow-auto">
          <div className="px-6 py-5 space-y-5">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input
                      ...
                      className={cn("h-9 text-sm", errors.firstName && "border-red-400 focus-visible:ring-red-200")}
                    />
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input
                      ...
                      className={cn("h-9 text-sm", errors.lastName && "border-red-400 focus-visible:ring-red-200")}
                    />
                  ...
                  <div className="flex items-center gap-3">
                    {form.profilePictureUrl ? (
                      <Avatar className="h-14 w-14 rounded-lg">
                        <AvatarImage src={form.profilePictureUrl} className="rounded-lg" />
            ...
            {/* ── Professional ─────────────────────────── */}
            <FormSection title="Professional Information" icon={Briefcase} defaultOpen>
              <div className="space-y-4">
                {/* Sports */}
                <MultiSelectChips
                  label="Sports *"
                ...
                {/* Description / About */}
                <div className="space-y-1.5">
                  <Label className="text-xs text-gray-500">Description / About</Label>
                  <Textarea
            ...
            {/* ── Qualifications ────────────────────────── */}
            <FormSection title="Qualifications" icon={GraduationCap} defaultOpen={false}>
              <div className="space-y-3">
                {form.qualifications.map((q, idx) => (
                  <div key={q.id} className="relative p-3 border border-gray-100 rounded-lg bg-gray-50/50">
                    <button
                      type="button"
                    ...
                    <div className="space-y-2 pr-6">
                      ...
                      <div className="space-y-1">
                        ...
                        <Textarea
                          value={q.description}
                          onChange={e => updateQualification(q.id, "description", e.target.value)}
            ...
            {/* ── Experience ────────────────────────────── */}
            <FormSection title="Experience" icon={Briefcase} defaultOpen={false}>
              <div className="space-y-3">
                {form.experiences.map((exp, idx) => (
                  <div key={exp.id} className="relative p-3 border border-gray-100 rounded-lg bg-gray-50/50">
                    <button
                      type="button"
                      className="absolute top-2 right-2 text-red-400 hover:text-red-600"
                          ...
                          <Popover>
                            ...
                            <PopoverContent className="w-auto p-0" align="start">
                              <CalendarUI
                                ...
                                onSelect={(d) => d && updateExperience(exp.id, "startDate", d)}
                                initialFocus
                              />
                            </PopoverContent>
                          </Popover>
                          ...
                          <Popover>
                            ...
                            <PopoverContent className="w-auto p-0" align="start">
                              <CalendarUI
                                ...
                                onSelect={(d) => d && updateExperience(exp.id, "endDate", d)}
                                initialFocus
                              />
                            </PopoverContent>
                          </Popover>
                        ...
                        <div className="flex items-center gap-2">
                          {exp.certificateFileName ? (
                            <Badge variant="secondary" className="text-[10px] gap-1">
                              ...
                              <X
                                className="h-2.5 w-2.5 cursor-pointer"
                                onClick={() => updateExperience(exp.id, "certificateFileName", "")}
                              />
                            </Badge>
                          ) : (
                            <label className="inline-flex items-center gap-1 px-2 py-1 text-[10px] border border-gray-200 rounded cursor-pointer hover:bg-gray-50 transition-colors">
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  const file = e.target.files?.[0];
                                  if (file) updateExperience(exp.id, "certificateFileName", file.name);
                                }}
                              />
                            </label>
                          )}
            ...
            <div className="flex items-center justify-between p-3 border border-gray-200 rounded-lg">
              <div>
                ...
                <p className="text-xs text-gray-400">
                  {form.status === "Active" ? "Coach is active and visible" : "Coach is inactive"}
                </p>
              </div>
              <div className="flex items-center gap-2">
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl max-h-[85vh] overflow-hidden flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>Detailed information about this coach.</DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-5 py-2">
            ...
            <div className="flex items-center gap-4">
              <Avatar className="h-16 w-16 rounded-xl">
                {coach.profilePictureUrl && (
                  <AvatarImage src={coach.profilePictureUrl} className="rounded-xl" />
                )}
              ...
              <div>
                <p className="text-base font-medium text-[#111827]">{getFullName(coach)}</p>
                <div className="flex items-center gap-2 mt-1">
                  <CoachStatusBadge status={coach.status} />
                  {coach.gender && (
                    <Badge variant="outline" className="text-[10px] text-gray-500">{coach.gender}</Badge>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 bg-gray-50 rounded-lg p-4">
              <div className="flex items-center gap-2">
                ...
                <a href={`mailto:${coach.email}`} className="text-sm text-[#003B95] hover:underline truncate">
                  {coach.email || "N/A"}
                </a>
              </div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-[#111827]">{coach.mobile || "N/A"}</span>
              </div>
              {coach.dateOfBirth && (
                <div className="flex items-center gap-2">
                  <Calendar className="h-3.5 w-3.5 text-gray-400" />
                  <span className="text-xs text-gray-500">DOB:</span>
                  <span className="text-sm text-[#111827]">{format(coach.dateOfBirth, "dd/MM/yyyy")}</span>
                </div>
              )}
              {coach.nationality && (
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-[#111827]">{coach.nationality}</span>
                </div>
              )}
              {coach.languages.length > 0 && (
                <div className="flex items-start gap-2 col-span-2">
                  ...
                  <span className="text-sm text-[#111827]">{coach.languages.join(", ")}</span>
                </div>
              )}
              <div className="flex items-center gap-2">
                <Calendar className="h-3.5 w-3.5 text-gray-400" />
                <span className="text-xs text-gray-500">Joined:</span>
                <span className="text-sm text-[#111827]">{format(coach.joinedAt, "dd/MM/yyyy")}</span>
              </div>
            </div>
            ...
            {coach.sports.length > 0 && (
              <div>
                <p className="text-xs font-medium text-gray-500 mb-1.5">Sports</p>
                <div className="flex flex-wrap gap-1.5">
                  {coach.sports.map(s => (
                    <Badge key={s} variant="outline" className="text-[10px] bg-blue-50 text-blue-700 border-blue-200">{s}</Badge>
                  ))}
                </div>
              </div>
            )}
              ...
              <div>
                ...
                <div className="space-y-2">
                  {coach.qualifications.map(q => (
                    <div key={q.id} className="p-2.5 bg-gray-50 rounded-lg border border-gray-100">
                      <p className="text-sm font-medium text-[#111827]">{q.title}</p>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {coach.experiences.map(e => (
                    <div key={e.id} className="p-2.5 bg-gray-50 rounded-lg border border-gray-100">
                      ...
                      <p className="text-[10px] text-gray-400 mt-1">
                        ...
                        {e.certificateFileName && (
                          <span className="ml-2 text-[#003B95]">{e.certificateFileName}</span>
                        )}
                      </p>
                    </div>
                  ))}
            ...
            {/* Tags */}
            {(coach.specialities.length > 0 || coach.skills.length > 0 || coach.areaOfExpertise.length > 0) && (
              <div className="space-y-2">
                {coach.specialities.length > 0 && (
                  <div>
                    <p className="text-xs font-medium text-gray-500 mb-1">Specialities</p>
...
export function CoachManagement({
  ...
  const handleAddSubmit = useCallback((data: CoachFormData) => {
  ...
  const handleEditSubmit = useCallback((data: CoachFormData) => {
    if (!editCoach) return;
  ...
  const isCoachAssigned = deleteTarget?.assignedTrainings && deleteTarget.assignedTrainings > 0;
    ...
    <div className="space-y-4">
      ...
      {coaches.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-12 bg-gray-50/50 rounded-lg border border-dashed border-gray-200">
          ...
          <p className="text-xs text-gray-400 mt-1">Click "Add Coach" to assign coaches to this provider</p>
          <Button
            className="mt-4 bg-[#003B95] hover:bg-[#002d73] text-white text-sm h-9"
            onClick={() => setAddOpen(true)}
          >
        ...
        <div className="flex flex-col items-center justify-center py-8 bg-gray-50/50 rounded-lg border border-gray-200">
          ...
          <p className="text-sm text-gray-500">No coaches match "{searchQuery}"</p>
        </div>
      ) : (
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-gray-50/80">
                <TableHead className="px-4 text-xs text-gray-500">Name</TableHead>
                <TableHead className="px-4 text-xs text-gray-500 hidden md:table-cell">Email</TableHead>
                <TableHead className="px-4 text-xs text-gray-500 hidden lg:table-cell">Sports</TableHead>
                <TableHead className="px-4 text-xs text-gray-500 hidden lg:table-cell">Gender</TableHead>
                <TableHead className="px-4 text-xs text-gray-500">Status</TableHead>
                <TableHead className="px-4 text-xs text-gray-500 text-center w-28">Actions</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
                  ...
                  <TableCell className="px-4">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div className="min-w-0">
                        <p className="text-sm text-[#111827] truncate">{getFullName(coach)}</p>
                        <p className="text-[11px] text-gray-400 md:hidden truncate">{coach.email}</p>
                      </div>
                    </div>
                  </TableCell>
                  <TableCell className="px-4 text-xs text-gray-500 hidden md:table-cell">
                    <a href={`mailto:${coach.email}`} className="hover:text-[#003B95]">
                      {coach.email}
                    </a>
                  </TableCell>
                  <TableCell className="px-4 hidden lg:table-cell">
                    <div className="flex flex-wrap gap-1">
                      ...
                      {coach.sports.length > 2 && (
                        <Badge variant="outline" className="text-[10px] text-gray-500">+{coach.sports.length - 2}</Badge>
                      )}
                    </div>
                  </TableCell>
      ...
      <CoachViewDialog
        open={!!viewCoach}
        onOpenChange={open => { if (!open) setViewCoach(null); }}
        coach={viewCoach}
      />
        ...
        <AlertDialogContent className="sm:max-w-md">
          <AlertDialogHeader>
            <AlertDialogTitle className="flex items-center gap-2">
              <AlertTriangle className={cn("h-5 w-5", isCoachAssigned ? "text-amber-500" : "text-red-500")} />
              {isCoachAssigned ? "Cannot Remove Coach" : "Remove Coach"}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {isCoachAssigned ? (
                <>
                  <span className="text-[#111827] font-medium">{deleteTarget && getFullName(deleteTarget)}</span>
                  {" "}is currently assigned to{" "}
                  <span className="text-[#111827] font-medium">{deleteTarget?.assignedTrainings} training(s)</span>.
                  You must unassign the coach from all trainings before removing them.
                </>
              ) : (
                <>
