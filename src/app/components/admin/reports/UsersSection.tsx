            ...
            <SafeChart height={250}>
              <LineChart data={userGrowth}>
                ...
                <Tooltip 
                  ...
                  formatter={(value: number) => [`${value} New Users`, 'Signups']}
                />
                <Line 
                  type="monotone" 
                  dataKey="signups" 
        ...
        <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm col-span-1 lg:col-span-1 min-w-0">
          ...
          <div className="h-[250px] w-full flex flex-col justify-center gap-4">
            {userRoles.map((role, index) => {
              ...
              const colors = ["bg-blue-600", "bg-green-500", "bg-purple-500", "bg-amber-500"];
              
              return (
                <div key={role.role} className="w-full">
                  <div className="flex justify-between text-sm mb-1">
                    <span className="font-medium text-gray-700">{role.role}</span>
                    <span className="text-gray-500">{role.count.toLocaleString()} ({percentage}%)</span>
                  </div>
