    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm col-span-1 lg:col-span-1 flex flex-col items-center min-w-0 overflow-hidden">
          <h3 className="text-lg font-medium text-gray-900 mb-4 self-start w-full">Bookings by Type</h3>
        ...
        <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm col-span-1 lg:col-span-1 min-w-0 overflow-hidden">
          ...
          <div className="space-y-4 relative">
            {bookingFunnel.map((step, index) => {
              const prevCount = index > 0 ? bookingFunnel[index - 1].count : step.count;
              const percentage = index === 0 ? 100 : Math.round((step.count / bookingFunnel[0].count) * 100);
              const dropOff = index > 0 ? ((prevCount - step.count) / prevCount * 100).toFixed(1) : 0;

              return (
                <div key={step.step} className="relative">
                  <div className="flex justify-between items-end mb-1">
                    <span className="text-sm font-medium text-gray-700">{step.step}</span>
                    <span className="text-xs font-semibold text-gray-900">{step.count.toLocaleString()}</span>
                  </div>
