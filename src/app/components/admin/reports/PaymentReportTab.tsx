...
export function PaymentReportTab({ filters }: Props) {
  const kpis = useMemo(
    () => PAYMENT_KPIS[filters.preset] ?? PAYMENT_KPIS["30d"],
    [filters.preset],
  );
  const revenueTrend = useMemo(
    () => PAYMENT_REVENUE_TREND[filters.preset] ?? PAYMENT_REVENUE_TREND["30d"],
    [filters.preset],
  );
  const refundTrend = useMemo(
    () => REFUND_TREND[filters.preset] ?? REFUND_TREND["30d"],
    [filters.preset],
  );
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5">
        <div className="mb-4">
          ...
          <p className="text-xs text-gray-400">
            AED — collected from players for the selected period
          </p>
        </div>
        <div className="h-[240px]">
          <SafeChart height={240}>
            ...
            >
              ...
              <Tooltip
                {...TOOLTIP_STYLE}
                formatter={(v: number) => [fmtAED(v), "Revenue"]}
              />
              <Line
                type="monotone"
                dataKey="value"
            ...
            <div className="relative h-[180px] w-[180px] shrink-0">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                ...
                <span className="text-lg text-gray-900 tabular-nums">
                  {kpis.successfulCount.toLocaleString()}
                </span>
              </div>
            </div>
            {/* Method rows */}
              ...
              <p className="text-lg text-emerald-600 tabular-nums">
                {(
                  (kpis.successfulCount /
                    (kpis.successfulCount + kpis.failedCount)) *
                  100
                ).toFixed(1)}
                %
              </p>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-gray-400">
              AED refunded to players — {kpis.refundsCount} total refunds
            </p>
          </div>
          <div className="bg-amber-50 border border-amber-100 rounded-lg px-3 py-1.5 text-center">
            ...
            <p className="text-sm text-amber-600 tabular-nums">
              {fmtAED(kpis.refundsAmount)}
            </p>
          </div>
        </div>
        <div className="h-[200px]">
          <SafeChart height={200}>
            ...
            >
              ...
              <Tooltip
                {...TOOLTIP_STYLE}
                formatter={(v: number) => [fmtAED(v), "Refunds"]}
              />
              <Bar
                dataKey="value"
                fill="#F59E0B"
