  ...
  return (
    <div className="fixed bottom-5 right-5 z-[100] flex flex-col gap-2 pointer-events-none">
      {toasts.map((t) => (
        <div
          key={t.id}
          className={`pointer-events-auto flex items-center gap-3 px-4 py-3 rounded-xl shadow-2xl text-sm font-medium text-white transition-all ${
            t.type === "success"
              ? "bg-emerald-600"
              : t.type === "warn"
                ? "bg-amber-600"
...
export function AdminListingsPage() {
  ...
  // Slide-over
  const [slideOverListing, setSlideOverListing] = useState<Listing | null>(
    null,
  );
  ...
  const listings = useMemo(
    () => allListings.filter((l) => l.type === activeTab),
  ...
  const handleRemoveConfirm = (listing: Listing, reason: string) => {
    const totalRefund = listing.futureBookings.reduce(
      (s, b) => s + b.amount,
      0,
    );
    const refundedBookings = listing.futureBookings.map((b) => ({
      ...b,
      refunded: true,
    }));
  ...
  const handleSuspendFromSlide = (l: Listing) => {
    setSuspendTarget(l);
  };
  const handleRemoveFromSlide = (l: Listing) => {
    setRemoveTarget(l);
  };
  ...
  const TABS: {
  ...
  }[] = [
    {
      ...
      count: stats.trainingTotal,
    },
    {
      id: "Facility",
      label: "Facility Listings",
      icon: <Building2 className="w-4 h-4" />,
      count: stats.facilityTotal,
    },
  ];
    ...
    <div className="p-6 space-y-5 bg-[#F9FAFB] min-h-screen">
      ...
      <div>
        ...
        <p className="text-gray-500 text-sm mt-1">
          View, suspend, and remove Training & Facility listings. Removal
          triggers full refunds for all future bookings.
        </p>
      </div>
