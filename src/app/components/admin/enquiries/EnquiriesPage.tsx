...
import {
  ...
  CalendarIcon, Send, Eye, Download,
} from "lucide-react";
...
type SortField = "id" | "submitterName" | "category" | "status" | "submittedAt" | "lastUpdated";
...
function withHistory(e: Enquiry, entry: Omit<StatusHistoryEntry, "id">): Enquiry {
  return { ...e, statusHistory: [...e.statusHistory, { ...entry, id: `sh-${Date.now()}` }] };
}
...
export function EnquiriesPage() {
  ...
  const activeFiltersCount = [catFilter !== "all", statusFilter !== "all", !!dateFrom, !!dateTo].filter(Boolean).length;
  ...
  const filtered = useMemo(() => enquiries.filter((e) => {
    ...
    if (dateFrom) { const d = new Date(dateFrom); d.setHours(0,0,0,0); if (e.submittedAt < d) return false; }
    if (dateTo)   { const d = new Date(dateTo);   d.setHours(23,59,59,999); if (e.submittedAt > d) return false; }
    return true;
  ...
  const sorted = useMemo(() => [...filtered].sort((a, b) => {
    ...
    switch (sortField) {
      ...
      case "submitterName": cmp = a.submitterName.localeCompare(b.submitterName); break;
      case "category":      cmp = a.category.localeCompare(b.category); break;
      case "status": {
        const ord: Record<EnquiryStatus, number> = { "Open": 0, "Resolved": 1 };
        cmp = ord[a.status] - ord[b.status]; break;
      }
      case "submittedAt":  cmp = a.submittedAt.getTime()  - b.submittedAt.getTime();  break;
      case "lastUpdated":  cmp = a.lastUpdated.getTime()  - b.lastUpdated.getTime();  break;
    }
  ...
  const handleSort = (f: SortField) => {
    ...
    else { setSortField(f); setSortDir(f === "submittedAt" || f === "lastUpdated" ? "desc" : "asc"); }
    setPage(1);
  };
  ...
  const quickResolve = (e: React.MouseEvent, enq: Enquiry) => {
    ...
    updated = withHistory(updated, { event: "Resolved", actor: CURRENT_ADMIN, timestamp: now, note: "Resolved via quick action in list" });
  ...
  const handleExport = () => {
    const rows = [
      ["Enquiry ID", "Submitter Name", "Email", "Account Type", "Category", "Subject", "Date Submitted", "Status", "Assigned To"],
      ...sorted.map((e) => [
        e.id, e.submitterName, e.submitterEmail, e.accountType,
        e.category, `"${e.subject.replace(/"/g, '""')}"`,
        format(e.submittedAt, "dd MMM yyyy HH:mm"),
        e.status, e.assignedTo ?? "Unassigned",
      ]),
    ];
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6 bg-[#F9FAFB] min-h-screen">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-[#111827]">Contact Us Enquiries</h1>
          <p className="text-sm text-[#6B7280] mt-1">
            Centralised inbox for all player, provider and guest enquiries. Respond, escalate, and resolve.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        <StatCard label="Total Enquiries"    value={stats.total}      icon={Inbox}        color="bg-blue-50 text-[#003B95]" />
        <StatCard label="Open"               value={stats.open}       icon={AlertTriangle} color={stats.open > 0 ? "bg-amber-50 text-amber-600" : "bg-gray-50 text-gray-400"}
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <Button variant="outline" onClick={() => setShowFilters(!showFilters)}
            ...
            {activeFiltersCount > 0 && (
              <Badge className="bg-[#003B95] text-white text-[10px] h-5 min-w-[20px]">{activeFiltersCount}</Badge>
            )}
          </Button>
        </div>
        ...
        {showFilters && (
          <div className="flex flex-wrap items-end gap-3 pt-3 border-t">
            ...
            <div className="space-y-1 min-w-[150px]">
              <label className="text-[11px] text-[#6B7280] uppercase tracking-wide">Status</label>
            ...
            <div className="space-y-1 min-w-[170px]">
              <label className="text-[11px] text-[#6B7280] uppercase tracking-wide">Category</label>
              <Select value={catFilter} onValueChange={(v) => { setCatFilter(v); setPage(1); }}>
                <SelectTrigger className="h-9"><SelectValue placeholder="All Categories" /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="all">All Categories</SelectItem>
                  {(["General Inquiry", "Technical Issue", "Booking Issue", "Payment Issue", "Feedback", "Other"] as EnquiryCategory[]).map((c) => (
            ...
            <div className="space-y-1">
              <label className="text-[11px] text-[#6B7280] uppercase tracking-wide">From</label>
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant="outline" className={cn("h-9 gap-2 text-sm min-w-[130px] justify-start", !dateFrom && "text-muted-foreground")}>
            ...
            {/* Date To */}
            <div className="space-y-1">
              <label className="text-[11px] text-[#6B7280] uppercase tracking-wide">To</label>
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant="outline" className={cn("h-9 gap-2 text-sm min-w-[130px] justify-start", !dateTo && "text-muted-foreground")}>
                    <CalendarIcon className="h-3.5 w-3.5" />
            ...
            {activeFiltersCount > 0 && (
              <Button variant="ghost" size="sm" onClick={clearFilters} className="gap-1.5 text-xs text-red-500 hover:text-red-700 h-9">
      ...
      <p className="text-xs text-[#9CA3AF]">
        Showing{" "}
        {sorted.length === 0 ? 0 : (safePage - 1) * PAGE_SIZE + 1}–{Math.min(safePage * PAGE_SIZE, sorted.length)}{" "}
        of <strong>{sorted.length}</strong> enquiries{activeFiltersCount > 0 && " (filtered)"}
      </p>
      ...
      <div className="bg-white border rounded-xl overflow-hidden">
          ...
          <TableHeader>
            <TableRow className="bg-gray-50/80 hover:bg-gray-50/80">
              <TableHead className="px-4 w-[150px]">
                <button onClick={() => handleSort("id")} className="flex items-center gap-1.5 text-xs hover:text-[#003B95]">
                  Enquiry ID <SortIcon field="id" />
                </button>
              </TableHead>
              <TableHead className="px-4">
                <button onClick={() => handleSort("submitterName")} className="flex items-center gap-1.5 text-xs hover:text-[#003B95]">
              ...
              <TableHead className="px-4">
                <button onClick={() => handleSort("category")} className="flex items-center gap-1.5 text-xs hover:text-[#003B95]">
              ...
              <TableHead className="px-4 hidden xl:table-cell">
                <button onClick={() => handleSort("submittedAt")} className="flex items-center gap-1.5 text-xs hover:text-[#003B95]">
                ...
                >
                  ...
                  <TableCell className="px-4">
                    <div className="space-y-0.5">
                      <span className="font-mono text-xs text-[#6B7280]">{enq.id}</span>
                      {enq.isGuest && (
                        <Badge variant="secondary" className="text-[9px] bg-gray-50 text-gray-500 border-gray-200 block w-fit">Guest</Badge>
                      )}
                    </div>
                  </TableCell>
                  ...
                  <TableCell className="px-4">
                    <div className="space-y-0.5">
                      <p className="text-sm text-[#111827] truncate max-w-[130px]">{enq.submitterName}</p>
                  ...
                  <TableCell className="px-4 hidden md:table-cell">
                    <span className="text-xs text-[#6B7280] truncate block max-w-[180px]">{enq.submitterEmail}</span>
                  </TableCell>

                  {/* Category */}
                  <TableCell className="px-4">
                    <Badge variant="secondary" className={cn("text-[10px] border whitespace-nowrap", catStyle.bg, catStyle.text, catStyle.border)}>
                      {enq.category}
                    </Badge>
                  </TableCell>
                  ...
                  <TableCell className="px-4 hidden lg:table-cell">
                    <p className="text-sm text-[#374151] max-w-[220px] truncate" title={enq.subject}>
                  ...
                  <TableCell className="px-4 hidden xl:table-cell">
                    <div>
                      <p className="text-xs text-[#374151]">{format(enq.submittedAt, "dd MMM yyyy")}</p>
                      <p className="text-[10px] text-[#9CA3AF]">{format(enq.submittedAt, "HH:mm")}</p>
                    </div>
                  </TableCell>

                  {/* Status badge — Open=red, In Progress=amber, Resolved=green */}
                  <TableCell className="px-4">
                    <Badge variant="secondary" className={cn("text-[10px] gap-1.5 whitespace-nowrap", sStyle.bg, sStyle.text)}>
                      <span className={cn("w-1.5 h-1.5 rounded-full", sStyle.dot)} />
                      {enq.status}
                    </Badge>
                    <p className="text-[10px] text-[#9CA3AF] mt-0.5">
                      {formatDistanceToNow(enq.lastUpdated, { addSuffix: true })}
                    </p>
                  </TableCell>
                  ...
                  <TableCell className="px-4" onClick={(e) => e.stopPropagation()}>
                    <div className="flex items-center gap-1 justify-center flex-wrap">
                      {/* View */}
                      <button onClick={() => openDetail(enq.id, false)}
                        title="View enquiry"
                      ...
                      {/* Reply */}
                      {!isResolved && (
                        <button onClick={() => openDetail(enq.id, true)}
                          title="Reply"
                      ...
                      {/* Mark as Resolved */}
                      {!isResolved && (
                        <button onClick={(e) => quickResolve(e, enq)}
              ...
              <TableRow>
                <TableCell colSpan={8} className="text-center py-16">
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <p className="text-sm text-[#9CA3AF]">No enquiries match your criteria</p>
                    <p className="text-xs text-[#D1D5DB]">Try adjusting your search or filters</p>
                  </div>
                </TableCell>
              </TableRow>
            )}
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between px-4 py-3 border-t bg-gray-50/50">
            <p className="text-xs text-[#9CA3AF]">Page {safePage} of {totalPages} · {sorted.length} results</p>
            <div className="flex items-center gap-1.5">
              <Button variant="outline" size="sm" disabled={safePage <= 1}
                onClick={() => setPage((p) => Math.max(1, p - 1))} className="h-7 w-7 p-0">
              ...
              {Array.from({ length: Math.min(totalPages, 7) }, (_, i) => {
                ...
                return (
                  <Button key={p} variant={p === safePage ? "default" : "outline"} size="sm"
                    onClick={() => setPage(p)}
                    className={cn("h-7 w-7 p-0 text-xs", p === safePage && "bg-[#003B95] hover:bg-[#002a6b]")}>
                    {p}
                  </Button>
                );
              })}
              <Button variant="outline" size="sm" disabled={safePage >= totalPages}
                onClick={() => setPage((p) => Math.min(totalPages, p + 1))} className="h-7 w-7 p-0">
      ...
      {selectedEnquiry && (
        <EnquiryDetailPanel
          ...
          onClose={() => { setSelectedId(null); setFocusReply(false); }}
