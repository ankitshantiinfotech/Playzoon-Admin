...
export function EnquiryDetailPanel({
  ...
  const replyValid =
    replyText.trim().length >= REPLY_MIN &&
  ...
  const handleEscalateConfirm = () => {
    ...
    const now = new Date();
    const memberName =
      ADMIN_TEAM.find((a) => a.id === escalateTarget)?.name ?? escalateTarget;
    let updated: Enquiry = {
      ...enquiry,
    ...
    setEscalateNote("");
    toast.success(
      `Enquiry escalated to ${memberName}. Push notification sent: "Enquiry [${enquiry.id}] has been assigned to you."`,
    );
  };
  ...
  const addInternalNote = () => {
    ...
    onUpdate({
      ...enquiry,
      internalNotes: [...enquiry.internalNotes, note],
      lastUpdated: now,
    });
    setNoteText("");
  ...
  const sortedNotes = [...enquiry.internalNotes].sort(
    (a, b) => a.timestamp.getTime() - b.timestamp.getTime(),
  );
  ...
  const AttachIcon = ({ type }: { type: string }) => {
    ...
    if (type === "video")
      return <VideoIcon className="h-4 w-4 text-purple-500 shrink-0" />;
    if (type === "pdf")
    ...
    <>
        ...
        <div className="flex items-start justify-between px-5 py-4 border-b shrink-0 bg-gray-50/60">
          <div className="space-y-2 min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              <span className="font-mono text-xs text-[#9CA3AF] bg-white border border-gray-200 px-1.5 py-0.5 rounded">
                {enquiry.id}
              </span>
              <Badge
                variant="secondary"
            ...
            <h2 className="text-sm text-[#111827] pr-8 leading-snug line-clamp-2">
              {enquiry.subject}
            </h2>
          </div>
          <button
            onClick={onClose}
            className="p-1.5 rounded-lg hover:bg-gray-200 text-gray-400 hover:text-gray-600 shrink-0 mt-0.5 transition-colors"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
          ...
          <div className="p-5 space-y-5">
              ...
              <div className="flex items-start gap-2.5 p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm">
                ...
                <div>
                  <span className="font-semibold text-slate-700">
                    Guest Submission (BR-124-07) —
                  </span>
                  <span className="text-slate-600">
                    {" "}
                    This enquiry was submitted without a platform account. Reply
                    will be sent via <strong>email only</strong>. No in-app
            ...
            <div className="bg-gray-50 rounded-xl p-4 space-y-2.5">
              <div className="flex items-center gap-3">
                <div className="w-9 h-10 rounded-full bg-[#003B95]/10 flex items-center justify-center text-[#003B95] shrink-0 select-none">
              ...
              <div className="grid grid-cols-1 gap-1.5 pl-12">
                <a
                  ...
                  className="flex items-center gap-2 text-xs text-[#003B95] hover:underline w-fit"
                >
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-3 text-[11px] text-[#9CA3AF]">
                ...
                <span>
                  (
                  {formatDistanceToNow(enquiry.submittedAt, {
                    addSuffix: true,
                  })}
                  )
                </span>
                ...
                <div className="flex flex-wrap gap-2">
                  {enquiry.attachments.map((att) => (
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        <p className="text-xs text-[#374151] truncate max-w-[150px] group-hover:text-[#003B95]">
                          {att.name}
                        </p>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <Select
                    value={enquiry.assignedTo ?? "__unassigned__"}
              ...
              <p className="text-[10px] text-[#9CA3AF] flex items-center gap-1">
                ...
                {formatDistanceToNow(enquiry.lastUpdated, { addSuffix: true })}
              </p>
            </div>
              ...
              <div className="flex border-b border-gray-200 mb-4">
                {TABS.map((t) => (
                  <button
                    ...
                    onClick={() => setTab(t.id)}
                    className={cn(
                      "flex-1 pb-2.5 text-xs font-medium border-b-2 transition-colors",
                      tab === t.id
                      ...
                      return (
                        <div
                          ...
                          className={cn(
                            ...
                            isAdmin
                              ? "bg-[#003B95]/5 border-[#003B95]/10 ml-0"
                              : "bg-white border-gray-200 mr-0",
                          )}
                        >
                          <div className="flex items-center justify-between gap-2">
                            <div className="flex items-center gap-1.5">
                              <div
                                className={cn(
                                  "w-6 h-6 rounded-full flex items-center justify-center text-[10px] text-white shrink-0",
                              ...
                              <span
                                className={cn(
                                  "text-xs font-medium",
                                  isAdmin ? "text-[#003B95]" : "text-[#374151]",
                                )}
                              >
                                {msg.authorName}
                              </span>
                              {isAdmin && (
                                <Badge
                                  variant="secondary"
                                  className="text-[9px] bg-[#003B95]/10 text-[#003B95] border-[#003B95]/20"
                                >
                                  Admin
                                </Badge>
                              )}
                            ...
                            <span className="text-[10px] text-[#9CA3AF] shrink-0">
                              {format(msg.timestamp, "MMM d, hh:mm a")}
                            </span>
                          </div>
                          <p className="text-sm text-[#374151] leading-relaxed whitespace-pre-wrap pl-7.5">
                            {msg.body}
                          </p>
                        </div>
                      );
                  ...
                  {sortedHistory.map((entry) => {
                    ...
                    return (
                      <div key={entry.id} className="relative">
                        <div
                          className={cn(
                            "absolute -left-[18px] top-1.5 w-3.5 h-3.5 rounded-full border-2 border-white",
                            eStyle.dot,
                          )}
                        />
                        <div className="space-y-0.5">
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            <span className="text-[10px] text-[#9CA3AF]">
                              {format(entry.timestamp, "MMM d, yyyy · hh:mm a")}
                            </span>
                          </div>
                      ...
                      >
                        <div className="flex items-center justify-between gap-2">
                          <div className="flex items-center gap-1.5">
                            ...
                            <Badge
                              variant="secondary"
                              className="text-[9px] bg-amber-100 text-amber-600 border-amber-300"
                            >
                              Internal
                            </Badge>
                          </div>
                          <span className="text-[10px] text-[#9CA3AF]">
                            {format(note.timestamp, "MMM d, hh:mm a")}
                          </span>
                        </div>
              ...
              {isResolved ? (
                ...
                <div className="flex items-start gap-2.5 p-3 bg-emerald-50 border border-emerald-200 rounded-xl text-sm">
                  ...
                  <div>
                    ...
                    <span className="text-emerald-700">
                      Re-open the enquiry to send new replies.
                    </span>
                  </div>
                </div>
              ) : (
                <>
                  <p className="text-[10px] text-[#9CA3AF]">
                    ...
                    {!enquiry.isGuest &&
                      ` and sent as in-app notification (subject: "Re: Your Enquiry [${enquiry.id}]")`}
                    {enquiry.isGuest &&
                      " (email only — guest has no platform account)"}
                  </p>
                  <Textarea
                    ref={replyRef}
                    value={replyText}
                  ...
                  <div className="flex items-center justify-between">
                    <p
                      className={cn(
                        "text-[10px]",
                        replyText.length > 0 && replyText.length < REPLY_MIN
                          ? "text-red-500"
            ...
            <div className="space-y-2.5">
              <h3 className="text-xs text-amber-600 flex items-center gap-1.5 uppercase tracking-wide">
                ...
                <span className="text-[10px] text-[#9CA3AF] normal-case ml-1 tracking-normal">
                  (admin-only)
                </span>
              </h3>
            ...
            <AlertDialogDescription asChild>
              <div className="space-y-2 text-sm text-gray-600">
                <p>
                  ...
                  <strong>{enquiry.submitterEmail}</strong> with subject:
                </p>
                <p className="font-mono bg-gray-50 border border-gray-200 rounded px-2 py-1 text-xs">
                  Re: Your Enquiry [{enquiry.id}]
                </p>
                {!enquiry.isGuest && (
                  <p>
                    The submitter will also receive an{" "}
                ...
                {enquiry.isGuest && (
                  <p className="flex items-center gap-1.5 text-slate-600 bg-slate-50 border border-slate-200 rounded p-2">
                    ...
                    Guest submission — email only, no in-app notification
                    (BR-124-07).
                  </p>
                )}
              </div>
        ...
        <AlertDialogContent className="z-[60]">
          <AlertDialogHeader>
            <AlertDialogTitle className="flex items-center gap-2">
              <CheckCircle2 className="h-5 w-5 text-emerald-600" /> Mark as
              Resolved?
            </AlertDialogTitle>
            <AlertDialogDescription asChild>
              <div className="space-y-2 text-sm text-gray-600">
                <p>
                  This will mark <strong>{enquiry.id}</strong> as{" "}
                  <strong>Resolved</strong>.
                </p>
                <p className="text-xs bg-amber-50 border border-amber-100 rounded p-2">
                  Per BR-124-04: A Resolved enquiry cannot receive further
                  replies unless re-opened. A "Re-open" button will appear.
                </p>
              </div>
            </AlertDialogDescription>
          </AlertDialogHeader>
      ...
      <AlertDialog open={escalateOpen} onOpenChange={setEscalateOpen}>
        <AlertDialogContent className="z-[60]">
          <AlertDialogHeader>
            <AlertDialogTitle className="flex items-center gap-2">
              <ShieldAlert className="h-5 w-5 text-orange-500" /> Escalate
              Enquiry
            </AlertDialogTitle>
            <AlertDialogDescription asChild>
              <div className="space-y-3 text-sm text-gray-600 mt-1">
                <p>
                  Select a Sub-Admin to assign and escalate{" "}
                  <strong>{enquiry.id}</strong> to. They will receive an in-app
                  push notification.
                </p>
                <div className="space-y-1.5">
                  ...
                  <Select
                    ...
                    onValueChange={setEscalateTarget}
                  >
                    <SelectTrigger>
                ...
                <p className="text-[11px] text-orange-700 bg-orange-50 border border-orange-100 rounded p-2 flex items-start gap-1.5">
                  ...
                  Push notification will be sent: "Enquiry [{enquiry.id}] has
