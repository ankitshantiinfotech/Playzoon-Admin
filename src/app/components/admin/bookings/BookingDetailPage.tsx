        ...
        >
          ...
          <div className="flex-1">
            <p className="text-sm font-semibold text-amber-800">
              This booking has been flagged as suspicious by{" "}
              {booking.flagInfo.flaggedBy} on{" "}
              {fmtDateTime(booking.flagInfo.flaggedAt)}.
            </p>
            <p className="text-sm text-amber-700 mt-1">
              Reason: {booking.flagInfo.reason}
            </p>
            <button
        ...
        <div className="col-span-3 space-y-6">
          ...
          >
            <div className="space-y-3">
              <div>
                ...
                <p className="text-xs text-[#6B7280]">
                  {booking.provider.type === "Facility"
                    ? "Facility Provider"
                    : booking.provider.type === "Training"
                      ? "Training Provider"
                      : booking.provider.type === "Coach"
                        ? "Freelance Coach"
                        : "Tournament Host"}{" "}
              ...
              {booking.provider.businessName && (
                <p className="text-sm text-[#6B7280]">
                  Business: {booking.provider.businessName}
                </p>
              )}
            </div>
          </Card>
          ...
          >
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              {booking.type === "Tournament" && (
                <>
                  <InfoField label="Entry Fee Type">
                    {booking.financials.grossAmount === 0 ? "Free" : "Paid"}
                  </InfoField>
                </>
              )}
            </div>
          ...
          <Card
            ...
            ariaLabel="Booking Configuration"
          >
            <div className="grid grid-cols-2 gap-4 text-sm">
              <InfoField label="Booking Date & Time">
                {fmtDateTime(booking.bookingDate)} UTC
              </InfoField>
              <InfoField label="Duration / Period">
                {booking.period} ({booking.duration} min)
              </InfoField>
              <InfoField label="Number of Players">
          ...
          <Card
            ...
            ariaLabel="Booking Status Timeline"
          >
            ...
            >
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                {booking.cancellation.cancellationPolicy && (
                  <InfoField label="Cancellation Policy">
                    {booking.cancellation.cancellationPolicy}
                  </InfoField>
                )}
          ...
          >
            {isFree ? (
              ...
              <div className="space-y-0">
                <FinancialRow
                  label="Gross Amount"
                  amount={fmtCurrency(booking.financials.grossAmount)}
                />
                ...
                <p className="text-[10px] text-[#6B7280] mt-3 italic">
                  Rates locked at booking time (Rule 69)
                </p>
              </div>
            )}
          </Card>
              ...
              <div className="space-y-3 text-sm">
                <InfoField label="Payment Method">
                  <span className="flex items-center gap-1.5">
                    <PaymentMethodIcon
                      method={booking.financials.paymentMethod}
                    />
                ...
                <InfoField label="Transaction ID">
                  <span className="font-mono text-xs text-[#003B95]">
                    {booking.financials.transactionId}
                  </span>
                </InfoField>
                <InfoField label="Payment Date">
                  {fmtDateTime(booking.financials.paidAt)} UTC
                </InfoField>
                <InfoField label="Gateway Reference">
                  <span className="font-mono text-xs text-[#6B7280]">
                    {booking.financials.gatewayRef}
                  </span>
                </InfoField>
              ...
              <div className="space-y-4">
                {booking.financials.refundHistory.map((refund) => (
                  <div key={refund.id} className="space-y-2 text-sm">
                    ...
                    <InfoField label="Refund Date">
                      {fmtDateTime(refund.processedAt)} UTC
                    </InfoField>
                    ...
                    {refund.method && (
                      <InfoField label="Refund Method">
                        {refund.method}
                      </InfoField>
                    )}
                    <InfoField label="Processed By">
                      {refund.processedBy}
                    </InfoField>
                  </div>
                ))}
      ...
      <Dialog open={flagModalOpen} onOpenChange={setFlagModalOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              You are about to flag booking <strong>{booking.id}</strong> as
              suspicious. Please provide a reason.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            <Textarea
...
function FinancialRow({
  ...
  return (
    ...
    >
      <span
        className={cn(bold ? "font-semibold text-[#111827]" : "text-[#6B7280]")}
      >
        {label}
      </span>
      <span
        className={cn(
          bold ? "font-bold text-[#111827]" : "text-[#111827]",
          className,
        )}
      >
        {amount}
