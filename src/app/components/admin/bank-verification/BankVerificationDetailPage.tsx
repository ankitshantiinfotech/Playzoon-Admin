...
function withHistory(
  r: BankVerification,
  entry: Omit<BVHistoryEntry, "id">,
): BankVerification {
  return {
    ...r,
    history: [...r.history, { ...entry, id: `he-${Date.now()}` }],
  };
}
...
export function BankVerificationDetailPage() {
  ...
  const [record, setRecord] = useState<BankVerification | null>(
    initialRecord ?? null,
  );
  ...
  // ── Validation ──
  const rejectValid =
    rejectReason.trim().length >= REASON_MIN &&
    rejectReason.trim().length <= REASON_MAX;
  const revokeValid =
    revokeReason.trim().length >= REASON_MIN &&
      ...
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="text-center space-y-4">
          ...
          <p className="text-sm text-[#6B7280] max-w-sm">
            Submission not found. It may have been removed or the URL is
            invalid.
          </p>
          <Button
            variant="outline"
  ...
  const providerRegDate =
    record.providerRegistrationDate ??
    new Date(record.submittedAt.getTime() - 30 * 86400000);
  const providerStatus = record.providerStatus ?? "Active";

  // ── Mock documents list ──
  const documents =
    record.documents ?? (record.document ? [record.document] : []);
  ...
  const confirmApprove = () => {
    ...
    toast.success(
      "Bank account approved. The provider is now eligible for payouts.",
    );
  };
  ...
  const confirmReject = () => {
    ...
    setRejectReason("");
    toast.success(
      "Bank account rejected. The provider has been notified with the rejection reason.",
    );
  };
  ...
  const submitRevocation = () => {
    ...
    setRevokeReason("");
    toast.success(
      "Revocation request submitted. A Super Admin must confirm before it takes effect.",
    );
  };
  ...
  return (
    <div
      ...
      role="main"
      aria-label="Bank Account Verification Detail"
    >
      {/* ── Breadcrumb ── */}
      <nav
        aria-label="Breadcrumb"
        className="text-sm text-[#6B7280] flex items-center gap-2"
      >
        <Button
          variant="ghost"
          size="sm"
        ...
        <div className="flex items-start gap-3 px-4 py-3 bg-blue-50 border border-blue-200 rounded-xl">
          ...
          <div>
            ...
            <p className="text-xs text-blue-700 mt-0.5">
              The revocation request has been submitted and is awaiting
              confirmation from a second Super Admin before it takes effect.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-5 gap-6">
        ...
        <div className="col-span-3 space-y-6">
          ...
          >
            <dl className="divide-y divide-gray-100">
              ...
              <InfoField label="Provider Type">
                <Badge
                  variant="outline"
                  className={cn("text-xs border", ts.bg, ts.text, ts.border)}
                >
                  {record.providerType}
                </Badge>
              </InfoField>
              <InfoField label="Email">
                <a
                  href={`mailto:${record.providerEmail}`}
                  className="text-[#003B95] hover:underline flex items-center gap-1"
                >
              ...
              <InfoField label="Registration Date">
                {format(providerRegDate, "dd MMM yyyy")}
              </InfoField>
          ...
          <DetailCard
            ...
            ariaLabel="Bank Account Details"
          >
            <dl className="divide-y divide-gray-100">
              ...
              <InfoField label="Submission Date">
                <div>
                  ...
                  <p className="text-[10px] text-[#9CA3AF]">
                    {formatDistanceToNow(record.submittedAt, {
                      addSuffix: true,
                    })}
                  </p>
                </div>
          ...
          <DetailCard
            ...
            ariaLabel="Submission Timeline"
          >
            <div
              className="relative border-l-2 border-gray-200 ml-3 pl-5 space-y-5 pb-1"
              role="list"
            >
              {sortedHistory.map((entry) => {
                ...
                return (
                  <div key={entry.id} className="relative" role="listitem">
                    <div
                      className={cn(
                        "absolute -left-[21px] top-1.5 w-4 h-4 rounded-full border-2 border-white",
                        es.dot,
                      )}
                    />
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className={cn("text-sm font-semibold", es.text)}>
                        {entry.event}
                      </span>
                    ...
                    <p className="text-xs text-[#6B7280] mt-0.5">
                      by{" "}
                      <strong className="text-[#111827]">{entry.actor}</strong>
                    </p>
                    {entry.note && (
                      <p className="text-xs text-[#6B7280] mt-0.5">
                        {entry.note}
            ...
            {isPending && !showRejectForm && (
              <div className="space-y-3">
                <p className="text-sm text-[#6B7280]">
                  Review the bank details and supporting documents, then approve
                ...
                <div>
                  ...
                  {rejectReason.trim().length > 0 &&
                    rejectReason.trim().length < REASON_MIN && (
                      ...
                      >
                        Rejection reason must be at least {REASON_MIN}{" "}
                        characters.
                      </p>
                    )}
                ...
                <div className="flex items-start gap-3 p-3 bg-emerald-50 border border-emerald-200 rounded-lg">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-emerald-700 mt-0.5">
                      Approved by <strong>{record.reviewedBy}</strong> on{" "}
                      {record.reviewedAt
                        ? format(record.reviewedAt, "dd MMM yyyy, HH:mm")
                        : "--"}{" "}
                      UTC. Payout processing is enabled for this provider.
                    </p>
                  </div>
                </div>
                <Button
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <Textarea
                    ...
                    value={revokeReason}
                    onChange={(e) =>
                      setRevokeReason(e.target.value.slice(0, REASON_MAX))
                    }
                  ...
                  {revokeReason.trim().length > 0 &&
                    revokeReason.trim().length < REASON_MIN && (
                      <p
                        ...
                        className="text-xs text-red-500 mt-1"
                      >
                        Revocation reason must be at least {REASON_MIN}{" "}
                        characters.
                      </p>
                    )}
                ...
                <div className="flex items-start gap-3 p-3 bg-red-50 border border-red-200 rounded-lg">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-red-800">
                      Rejected by {record.reviewedBy} on{" "}
                      {record.reviewedAt
                        ? format(record.reviewedAt, "dd MMM yyyy")
                        : "--"}
                    </p>
                    ...
                    >
                      ...
                      <div className="flex-1">
                        <div className="flex items-center justify-between gap-2">
                          <p className={cn("font-medium", es.text)}>
                            {entry.event}
                          </p>
        ...
        <AlertDialogContent role="alertdialog">
          <AlertDialogHeader>
            <AlertDialogTitle className="flex items-center gap-2">
              <CheckCircle2 className="w-5 h-5 text-amber-600" /> Approve Bank
              Account?
            </AlertDialogTitle>
            <AlertDialogDescription asChild>
              <div className="space-y-3 text-sm text-[#6B7280] mt-1">
                <p>
                  This action is irreversible. The provider will become eligible
                  for payouts. Confirm approval?
                </p>
                <div className="bg-gray-50 border border-gray-200 rounded-xl p-3 space-y-1.5">
                  ...
                  <p className="text-xs text-[#6B7280] font-mono tracking-wider mt-1">
                    IBAN: {formatIBAN(record.ibanFull)}
                  </p>
                </div>
              </div>
            </AlertDialogDescription>
            ...
            <div className="bg-white rounded-xl shadow-2xl p-8 w-full aspect-[4/3] flex items-center justify-center">
              {documents[viewerDocIndex]?.type === "image" ? (
                <div className="text-center">
                  ...
                  <p className="text-sm text-[#6B7280]">
                    Image preview: {documents[viewerDocIndex]?.name}
                  </p>
                </div>
              ) : (
                <div className="text-center">
                  ...
                  <p className="text-sm text-[#6B7280]">
                    PDF preview: {documents[viewerDocIndex]?.name}
                  </p>
                </div>
              )}
            </div>
            {documents.length > 1 && (
              <div className="flex items-center gap-3">
                ...
                <Button
                  ...
                  onClick={() =>
                    setViewerDocIndex((i) =>
                      Math.min(documents.length - 1, i + 1),
                    )
                  }
