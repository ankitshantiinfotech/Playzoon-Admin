...
import {
  ...
  Clock, CheckCircle2, XCircle, BanknoteIcon,
} from "lucide-react";
...
import {
  BVStatus, BVProviderType,
  BV_STATUS_STYLES, BV_PROVIDER_TYPE_STYLES,
} from "./types";
...
type SortField = "providerName" | "providerType" | "bankName" | "submissionDate" | "status";
type SortDir = "asc" | "desc";

// ─── Stat Card ──────────────────────────────────────────────────────────────

function StatCard({
  label, count, color, bgColor, borderColor, icon: Icon, isActive, onClick,
}: {
  label: string;
  count: number;
  color: string;
...
export function AdminBankVerificationPage() {
  ...
  const hasActiveFilters = statusFilter !== "all" || providerTypeFilter !== "all" || !!dateFrom || !!dateTo || !!debouncedSearch;
  ...
  const filtered = useMemo(() => {
    ...
    result.sort((a, b) => {
      ...
      switch (sortField) {
        ...
        case "providerType": cmp = a.providerType.localeCompare(b.providerType); break;
        case "bankName": cmp = a.bankName.localeCompare(b.bankName); break;
        case "submissionDate": cmp = a.submittedAt.getTime() - b.submittedAt.getTime(); break;
    ...
    return result;
  }, [records, debouncedSearch, statusFilter, providerTypeFilter, dateFrom, dateTo, sortField, sortDir]);
  ...
  // ── Selection helpers ──────────────────────────────
  const allVisibleSelected = paged.length > 0 && paged.every(r => selectedIds.has(r.id));
  ...
  const handleExport = (fmt: string) => {
    ...
    toast.success(`Export started. Your file will download shortly. (${fmt.toUpperCase()})`);
  };
  ...
  const ProviderTypeBadge = ({ type }: { type: BVProviderType }) => {
    ...
    return (
      <Badge variant="outline" className={cn("text-xs border", styles.bg, styles.text, styles.border)}>
        {type}
      </Badge>
    );
  };
  ...
  return (
    <div className="flex flex-col gap-6 max-w-[1400px]" role="main" aria-label="Bank Account Verification">
      ...
      <nav aria-label="Breadcrumb" className="text-sm text-[#6B7280]">
        ...
        <span className="mx-2">/</span>
        <span className="text-[#111827] font-medium">Bank Account Verification</span>
      </nav>

      {/* ── Page Header ── */}
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <h1 className="text-2xl font-bold tracking-tight text-[#111827]">Bank Account Verification</h1>
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            <DropdownMenuItem onClick={() => handleExport("csv")} aria-label="Export as CSV">CSV</DropdownMenuItem>
            <DropdownMenuItem onClick={() => handleExport("xls")} aria-label="Export as XLS">XLS</DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      </div>
      ...
      <div className="bg-white border rounded-xl p-4 space-y-3">
        <div className="flex flex-wrap items-end gap-3">
          {/* Search */}
          <div className="relative flex-1 min-w-[280px] max-w-[320px]" role="search">
          ...
          <div className="min-w-[170px]">
            <Select value={providerTypeFilter} onValueChange={(v) => { setProviderTypeFilter(v as ProviderTypeFilter); setPage(1); }}>
              <SelectTrigger className="h-9" aria-label="Filter by provider type">
          ...
          {/* Date Range */}
          <div className="flex items-center gap-2" aria-label="Filter by submission date range">
            <div className="flex items-center gap-1">
              ...
              <input
                ...
                className="text-sm border border-gray-200 rounded-lg py-1.5 px-2 h-9 bg-white focus:ring-[#003B95] focus:outline-none focus:border-[#003B95]"
              />
            </div>
            <div className="flex items-center gap-1">
          ...
          {/* Clear Filters */}
          {hasActiveFilters && (
            <Button variant="ghost" size="sm" onClick={clearAllFilters} className="gap-1.5 text-xs text-red-500 hover:text-red-700 h-9">
      ...
      {/* ── Bulk Action Bar ── */}
      {selectedIds.size > 0 && (
        <div className="bg-blue-50 border border-blue-200 rounded-xl px-4 py-3 flex items-center justify-between" aria-live="polite">
          <span className="text-sm text-blue-800 font-medium">{selectedIds.size} row{selectedIds.size !== 1 ? "s" : ""} selected</span>
          <Button variant="ghost" size="sm" onClick={() => setSelectedIds(new Set())} className="text-blue-600 hover:text-blue-800">
            Clear Selection
          </Button>
        </div>
      )}
      ...
      {/* ── DataTable ── */}
      <div className="bg-white border rounded-xl overflow-hidden" role="table" aria-label="Bank account submissions table">
          ...
          <table className="w-full text-sm text-left">
            <thead className="text-xs text-[#6B7280] uppercase bg-gray-50 border-b border-gray-200 sticky top-0">
              <tr>
                ...
                <th className="px-3 py-3 w-[180px] cursor-pointer hover:bg-gray-100" onClick={() => handleSort("providerName")}>
                  Provider Name <SortIcon col="providerName" />
                </th>
                <th className="px-3 py-3 w-[140px] cursor-pointer hover:bg-gray-100" onClick={() => handleSort("providerType")}>
                  Provider Type <SortIcon col="providerType" />
                </th>
                <th className="px-3 py-3 w-[160px] cursor-pointer hover:bg-gray-100" onClick={() => handleSort("bankName")}>
                  Bank Name <SortIcon col="bankName" />
                </th>
                <th className="px-3 py-3 w-[200px]">IBAN</th>
                <th className="px-3 py-3 w-[150px] cursor-pointer hover:bg-gray-100" onClick={() => handleSort("submissionDate")}>
                  Submission Date <SortIcon col="submissionDate" />
                </th>
                <th className="px-3 py-3 w-[120px] cursor-pointer hover:bg-gray-100" onClick={() => handleSort("status")}>
                ...
                >
                  ...
                  <td className="px-3 py-3">
                    <span
                      ...
                      onClick={(e) => { e.stopPropagation(); navigate(`/bank-verification/${record.id}`); }}
                    >
                      {record.providerName}
                    </span>
                  </td>
                  ...
                  {/* Bank Name */}
                  <td className="px-3 py-3">
                    <span className="text-sm text-[#111827] truncate block max-w-[150px]" title={record.bankName}>
                      {record.bankName}
                    </span>
                  </td>
                  ...
                  <td colSpan={8} className="text-center py-16">
                    {hasActiveFilters ? (
                      <>
                        ...
                        <p className="text-sm font-medium text-[#111827]">No Bank Account Submissions</p>
                        <p className="text-xs text-[#6B7280] mt-1">No bank account submissions match your current filters.</p>
                      </>
                    ) : (
                      <>
                        ...
                        <p className="text-sm font-medium text-[#111827]">No Bank Account Submissions</p>
                        <p className="text-xs text-[#6B7280] mt-1">Bank account submissions from providers will appear here once submitted.</p>
                      </>
                    )}
                  </td>
                </tr>
