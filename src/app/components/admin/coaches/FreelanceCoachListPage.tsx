...
import {
  Search, X, Download, Eye, ChevronUp, ChevronDown,
  Star, ShieldCheck, ShieldX, Clock, CheckCircle2,
} from "lucide-react";
...
type SortField = "id" | "name" | "experience" | "rating" | "status" | "verificationStatus" | "registeredDate";
...
const VERIFICATION_STYLES: Record<string, { cls: string; icon: React.ElementType | null }> = {
  Verified: { cls: "bg-emerald-100 text-emerald-800 border-emerald-200", icon: ShieldCheck },
  Unverified: { cls: "bg-gray-100 text-gray-600 border-gray-200", icon: ShieldX },
...
function RatingStars({ rating, reviewCount }: { rating: number; reviewCount: number }) {
  ...
  return (
    <span className="flex items-center gap-1" role="img" aria-label={`Rated ${rating} out of 5 stars`}>
      <span className="flex">{stars}</span>
      <span className="text-xs text-[#111827] font-medium">{rating.toFixed(1)}</span>
    </span>
  );
}
...
function AvatarInitials({ firstName, lastName }: { firstName: string; lastName: string }) {
  ...
  return (
    <div className={cn("w-9 h-9 rounded-full flex items-center justify-center text-white text-xs font-bold shrink-0", bg)} aria-hidden="true">
...
export function FreelanceCoachListPage() {
  ...
  if (sportFilter !== "all") activeFilters.push({ label: `Sport: ${sportFilter}`, onClear: () => { setSportFilter("all"); setPage(1); } });
  if (statusFilter !== "all") activeFilters.push({ label: `Status: ${statusFilter.charAt(0).toUpperCase() + statusFilter.slice(1)}`, onClear: () => { setStatusFilter("all"); setPage(1); } });
  if (verificationFilter !== "all") activeFilters.push({ label: `Verification: ${verificationFilter.charAt(0).toUpperCase() + verificationFilter.slice(1)}`, onClear: () => { setVerificationFilter("all"); setPage(1); } });
  if (ratingFilter !== "all") activeFilters.push({ label: `Rating: ${ratingFilter === "unrated" ? "Unrated" : ratingFilter + " Stars"}`, onClear: () => { setRatingFilter("all"); setPage(1); } });
  ...
  const filtered = useMemo(() => {
    ...
    if (verificationFilter !== "all") {
      result = result.filter(c => c.verificationStatus.toLowerCase() === verificationFilter);
    }
    ...
    result.sort((a, b) => {
      ...
      switch (sortField) {
        ...
        case "verificationStatus": cmp = a.verificationStatus.localeCompare(b.verificationStatus); break;
        case "registeredDate": cmp = a.registeredDate.getTime() - b.registeredDate.getTime(); break;
      }
  ...
  return (
    <div className="flex flex-col gap-6 max-w-[1400px]" role="main" aria-label="Coach Management">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <h1 className="text-2xl font-bold tracking-tight text-[#111827]">Coach Management</h1>
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="outline" className="gap-2" aria-label="Export coach data" disabled={filtered.length === 0}>
              <Download className="h-4 w-4" /> Export
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end">
            <DropdownMenuItem onClick={() => handleExport("csv")}>CSV</DropdownMenuItem>
            <DropdownMenuItem onClick={() => handleExport("xls")}>XLS</DropdownMenuItem>
            <DropdownMenuItem onClick={() => handleExport("pdf")}>PDF</DropdownMenuItem>
            <DropdownMenuItem onClick={() => handleExport("txt")}>TXT</DropdownMenuItem>
            <DropdownMenuItem onClick={() => handleExport("sql")}>SQL</DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      </div>
        ...
        <div className="flex flex-wrap items-end gap-3">
          ...
          <div className="space-y-1 min-w-[160px]">
            <Select value={sportFilter} onValueChange={(v) => { setSportFilter(v); setPage(1); }}>
              <SelectTrigger className="h-9" aria-label="Filter by sport">
          ...
          <div className="space-y-1 min-w-[140px]">
            <Select value={statusFilter} onValueChange={(v) => { setStatusFilter(v as StatusFilter); setPage(1); }}>
              <SelectTrigger className="h-9" aria-label="Filter by coach status">
          ...
          <div className="space-y-1 min-w-[160px]">
            <Select value={verificationFilter} onValueChange={(v) => { setVerificationFilter(v as VerificationFilter); setPage(1); }}>
              <SelectTrigger className="h-9" aria-label="Filter by verification status">
          ...
          <div className="space-y-1 min-w-[130px]">
            <Select value={ratingFilter} onValueChange={(v) => { setRatingFilter(v as RatingFilter); setPage(1); }}>
              <SelectTrigger className="h-9" aria-label="Filter by minimum rating">
          ...
          {activeFilters.length > 0 && (
            <Button variant="ghost" size="sm" onClick={clearAllFilters} className="gap-1.5 text-xs text-red-500 hover:text-red-700 h-9" aria-label="Clear all active filters">
        ...
        {activeFilters.length > 0 && (
          <div className="flex flex-wrap items-center gap-2 pt-3 border-t">
            {activeFilters.map((f, i) => (
              <Badge key={i} variant="secondary" className="gap-1.5 px-2.5 py-1 text-xs bg-blue-50 text-blue-700 border border-blue-200">
                {f.label}
                <button onClick={f.onClear} className="ml-1 hover:text-blue-900">
                  <X className="h-3 w-3" />
                </button>
              </Badge>
            ))}
            <button onClick={clearAllFilters} className="text-xs text-red-500 hover:text-red-700 hover:underline ml-2">
              Clear All
            </button>
          </div>
        )}
      ...
      {/* ── DataTable ── */}
      <div className="bg-white border rounded-xl overflow-hidden" role="table" aria-label="Coaches table">
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="text-xs text-[#6B7280] uppercase bg-gray-50 border-b border-gray-200 sticky top-0">
              <tr>
                <th className="px-3 py-3 w-[100px] cursor-pointer hover:bg-gray-100" onClick={() => handleSort("id")}>
                  ID <SortIcon col="id" />
                </th>
                <th className="px-2 py-3 w-[50px]">Avatar</th>
                <th className="px-3 py-3 w-[180px] cursor-pointer hover:bg-gray-100" onClick={() => handleSort("name")}>
                  Name <SortIcon col="name" />
                </th>
                <th className="px-3 py-3 w-[150px]">Sports</th>
                <th className="px-3 py-3 w-[150px]">Specialities</th>
                <th className="px-3 py-3 w-[100px] cursor-pointer hover:bg-gray-100" onClick={() => handleSort("experience")}>
                  Experience <SortIcon col="experience" />
                </th>
                <th className="px-3 py-3 w-[100px] cursor-pointer hover:bg-gray-100" onClick={() => handleSort("rating")}>
                  Rating <SortIcon col="rating" />
                </th>
                <th className="px-3 py-3 w-[100px] cursor-pointer hover:bg-gray-100" onClick={() => handleSort("status")}>
                  Status <SortIcon col="status" />
                </th>
                <th className="px-3 py-3 w-[120px] cursor-pointer hover:bg-gray-100" onClick={() => handleSort("verificationStatus")}>
                  Verification <SortIcon col="verificationStatus" />
                </th>
                <th className="px-3 py-3 w-[120px] cursor-pointer hover:bg-gray-100" onClick={() => handleSort("registeredDate")}>
            ...
            <tbody className="divide-y divide-gray-100">
              {paged.length > 0 ? paged.map((coach, idx) => (
                <tr
                  key={coach.id}
                  className={cn(
                ...
                >
                  ...
                  <td className="px-2 py-3">
                    <AvatarInitials firstName={coach.firstName} lastName={coach.lastName} />
                  </td>

                  {/* Name */}
                  <td className="px-3 py-3">
                    <span className="text-sm font-medium text-[#111827] truncate block max-w-[170px]" title={`${coach.firstName} ${coach.lastName}`}>
                  ...
                  {/* Rating */}
                  <td className="px-3 py-3">
                    <RatingStars rating={coach.rating} reviewCount={coach.reviewCount} />
                  </td>

                  {/* Status */}
                  <td className="px-3 py-3">
                    <Badge variant="outline" className={cn("text-xs border", STATUS_STYLES[coach.status])}>
                      {coach.status}
                    </Badge>
                  </td>
                  ...
                  <td className="px-3 py-3">
                    {(() => {
                      ...
                      const Icon = v.icon;
                      return (
                        <Badge variant="outline" className={cn("text-xs border gap-1", v.cls)}>
                  ...
                  <td colSpan={11} className="text-center py-16">
                    {debouncedSearch || activeFilters.length > 0 ? (
                      <>
                        ...
                        <p className="text-sm font-medium text-[#111827]">No Coaches Found</p>
                        <p className="text-xs text-[#6B7280] mt-1">No coaches match your current filters.</p>
                      </>
                    ) : (
                      <>
                        ...
                        <p className="text-sm font-medium text-[#111827]">No Coaches Yet</p>
                        <p className="text-xs text-[#6B7280] mt-1">No freelance coaches have been registered on the platform yet.</p>
                      </>
                    )}
                  </td>
                </tr>
      ...
      {/* ── Pagination ── */}
      <nav aria-label="Coaches table pagination" className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1.5">
            ...
            {[10, 20, 50].map(ps => (
              <Button
                ...
                className={cn("h-6 px-2 text-[10px]", ps === pageSize && "bg-[#003B95] hover:bg-[#002d73]")}
              >
                {ps}
              </Button>
            ))}
        ...
        {totalPages > 1 && (
          <div className="flex items-center gap-1.5">
            <Button variant="outline" size="sm" disabled={safePage <= 1} onClick={() => setPage(p => Math.max(1, p - 1))} className="h-7 w-7 p-0">
              &lsaquo;
            </Button>
            {Array.from({ length: Math.min(totalPages, 5) }, (_, i) => {
              ...
              return (
                <Button key={p} variant={p === safePage ? "default" : "outline"} size="sm"
                  onClick={() => setPage(p)}
                  className={cn("h-7 w-7 p-0 text-xs", p === safePage && "bg-[#003B95] hover:bg-[#002d73]")}>
                  {p}
                </Button>
              );
            })}
            <Button variant="outline" size="sm" disabled={safePage >= totalPages} onClick={() => setPage(p => Math.min(totalPages, p + 1))} className="h-7 w-7 p-0">
              &rsaquo;
            </Button>
          </div>
        )}
...
function ChipList({ items, max }: { items: string[]; max: number }) {
  ...
  return (
    <div className="flex flex-wrap gap-1">
      {shown.map(item => (
        <span key={item} className="inline-block px-1.5 py-0.5 bg-gray-100 text-gray-700 text-[10px] rounded border border-gray-200">
          {item}
        </span>
      ))}
      {remaining > 0 && (
