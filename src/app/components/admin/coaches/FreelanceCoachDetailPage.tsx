...
function buildMockDetail(c: FreelancerCoach): CoachDetail {
  const hash =
    c.id.charCodeAt(c.id.length - 1) + c.id.charCodeAt(c.id.length - 2);
  const isApproved = c.verificationStatus === "Approved";
  const totalBookings = isApproved ? (hash % 80) + 10 : 0;
  const rating = isApproved
    ? Math.round((3.5 + (hash % 15) / 10) * 10) / 10
    : 0;
  ...
  return {
    ...
    sports: [
      ...
      ...(hash % 3 === 0
        ? [{ name: "Fitness & Conditioning", proficiency: "Intermediate" }]
        : []),
    ],
    specialities: [
      c.specialization === "Football"
        ? "Tactical Coaching"
        : c.specialization === "Tennis"
          ? "Serve Improvement"
          : "General Training",
    ...
    qualifications: isApproved
      ? [
          {
            name: `${c.specialization} Coaching Certificate`,
            issuingBody: "National Sports Authority",
            year: 2020,
          },
          ...(hash % 3 === 0
            ? [
    ...
    availability: [
      ...
      },
      {
        day: "Wednesday",
        available: hash % 2 === 0,
        start: hash % 2 === 0 ? "09:00" : undefined,
        end: hash % 2 === 0 ? "17:00" : undefined,
        maxSessions: hash % 2 === 0 ? 4 : undefined,
      },
      {
        day: "Thursday",
        available: true,
    ...
    wallet: {
      currentBalance: isApproved
        ? Math.round((hash % 20) * 100 * 100) / 100
        : 0,
      totalEarned: totalEarnings,
      totalWithdrawn: isApproved
        ? Math.round(totalEarnings * 0.7 * 100) / 100
        : 0,
      pendingPayout:
        isApproved && hash % 4 === 0
          ? Math.round((hash % 10) * 50 * 100) / 100
          : 0,
      payoutStatus:
        isApproved && hash % 4 === 0 ? "Payout Requested" : "No Pending Payout",
    },
    verificationDocs: [
      {
        docType: "ID Document",
        ...
        rejectionReason:
          c.verificationStatus === "Rejected" ? c.rejectionReason : undefined,
      },
      {
    ...
    recentBookings: isApproved
      ? Array.from({ length: Math.min(5, totalBookings) }, (_, i) => ({
          bookingId: `BK-${1000 + i}`,
          playerName: [
            "Mohammed Ali",
            "Sarah Ahmed",
      ...
      : [],
    reviews: isApproved
      ? Array.from({ length: Math.min(5, totalReviews) }, (_, i) => ({
          id: `rev-${i}`,
          reviewerName: [
            "Mohammed Ali",
            "Sarah Ahmed",
    ...
    auditTrail: [
      {
        ...
        event: "Coach registered",
        actor: "System",
        details: "Self-registration via provider portal",
      },
      {
        id: "at-2",
        timestamp: new Date(c.createdAt.getTime() + 86400000),
        event: "Verification documents submitted",
        actor: getCoachFullName(c),
      },
      ...(c.verificationStatus === "Approved"
        ? [
            {
              id: "at-3",
              timestamp: new Date(c.createdAt.getTime() + 86400000 * 3),
      ...
      ...(c.accountStatus === "Locked"
        ? [
            {
              id: "at-5",
              timestamp: c.lockedAt ?? new Date(),
              event: "Account locked",
              actor: c.lockedBy ?? "System",
            },
          ]
        : []),
    ],
...
function InfoField({
  ...
  return (
    <div>
      <dt className="text-xs text-[#6B7280] uppercase tracking-wide mb-0.5">
        {label}
      </dt>
      ...
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="text-center space-y-4">
          ...
          <p className="text-sm text-[#6B7280] max-w-sm">
            The coach you are looking for does not exist or the ID may be
            invalid.
          </p>
          <Button
            variant="outline"
  ...
  const verStatusLabel =
    c.verificationStatus === "Approved"
      ...
      : c.verificationStatus === "Pending"
        ...
        : "Unverified";
  const statusLabel =
    c.accountStatus === "Locked" ? "Locked" : c.platformStatus;
    ...
    >
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6">
        <div className="flex items-start gap-5">
          {/* Avatar */}
          <div
            className="w-20 h-20 rounded-full bg-[#003B95]/10 flex items-center justify-center text-[#003B95] text-2xl font-bold shrink-0"
            role="img"
            ...
            <div className="flex items-start justify-between gap-4 flex-wrap">
              <div>
                ...
                <div className="flex items-center gap-2 mt-2 flex-wrap">
                  <Badge
                    variant="outline"
                    className={cn("text-xs border", STATUS_STYLES[statusLabel])}
                  >
                    {statusLabel}
                  </Badge>
                  <Badge
                    ...
                    className={cn(
                      ...
                      VER_STYLES[verStatusLabel],
                    )}
                  >
                    {verStatusLabel === "Verified" && (
                      <ShieldCheck className="w-3 h-3" />
                    )}
                    {verStatusLabel === "Unverified" && (
                      <ShieldX className="w-3 h-3" />
                    )}
                    {verStatusLabel === "Pending" && (
              ...
              <Button
                variant="ghost"
                size="sm"
                className="gap-1.5 text-[#003B95] hover:text-[#002d73]"
                aria-label="Edit coach profile in Provider Management"
                onClick={() => navigate(`/providers/coach/${c.id}/edit`)}
              >
                <ExternalLink className="h-3.5 w-3.5" /> Edit in Provider
                Management
              </Button>
            </div>
          </div>
        ...
        {c.platformStatus === "Inactive" && c.accountStatus !== "Locked" && (
          <div className="mt-4 p-3 bg-gray-50 border border-gray-200 rounded-lg text-sm text-gray-700">
            This coach is currently inactive and hidden from player-facing
            listings.
          </div>
        )}
        {c.accountStatus === "Locked" && (
          <div className="mt-4 p-3 bg-red-50 border border-red-200 rounded-lg text-sm text-red-700 flex items-center gap-2">
            ...
            This coach account is locked.
            {c.lockedBy && ` Locked by ${c.lockedBy}.`}
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-5 gap-6">
        ...
        <div className="col-span-3 space-y-6">
          ...
          >
            <dl className="grid grid-cols-2 gap-4">
              ...
              <InfoField label="Date of Birth">
                {format(c.dob, "dd MMM yyyy")} ({age} years)
              </InfoField>
            ...
            <div className="mt-4 pt-4 border-t border-gray-100">
              <dt className="text-xs text-[#6B7280] uppercase tracking-wide mb-1">
                About Me / Bio
              </dt>
          ...
          <Card
            ...
            ariaLabel="Professional Details"
          >
            <dl className="grid grid-cols-2 gap-4">
              ...
              <InfoField label="Experience">
                {detail.experience} years of coaching experience
              </InfoField>
            </dl>
            <div className="mt-4 pt-4 border-t border-gray-100">
              <dt className="text-xs text-[#6B7280] uppercase tracking-wide mb-2">
                Qualifications
              </dt>
              {detail.qualifications.length > 0 ? (
                ...
                <dd className="text-sm text-[#9CA3AF]">
          ...
          <Card
            ...
            ariaLabel="Availability Schedule"
          >
                  ...
                  {detail.availability.map((a) => (
                    <tr key={a.day}>
                      ...
                      <td className="py-2 pr-4 text-[#6B7280]">
                        {a.available ? `${a.start} UTC` : "--"}
                      </td>
                      <td className="py-2 pr-4 text-[#6B7280]">
                        {a.available ? `${a.end} UTC` : "--"}
                      </td>
                      <td className="py-2 text-[#6B7280]">
                        {a.available ? a.maxSessions : "--"}
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          >
            {detail.pricing ? (
              <dl className="grid grid-cols-2 gap-4">
                <InfoField label="Per Session Rate">
                  {fmtCurrency(detail.pricing.perSession)} / session
                </InfoField>
                <InfoField label="Session Duration">
                  {detail.pricing.sessionDuration} minutes
                </InfoField>
                {detail.pricing.package5 && (
                  <InfoField label="5-Session Package">
                    {fmtCurrency(detail.pricing.package5)}
                  </InfoField>
                )}
                {detail.pricing.package10 && (
                  <InfoField label="10-Session Package">
                    {fmtCurrency(detail.pricing.package10)}
                  </InfoField>
                )}
                <InfoField label="Trial Session">
                  {detail.pricing.trialSession === "Free"
                    ? "Free Trial Available"
                    : detail.pricing.trialSession
                      ? fmtCurrency(detail.pricing.trialSession)
                      : "Not offered"}
                </InfoField>
              </dl>
            ) : (
        ...
        <div className="col-span-2 space-y-6">
          ...
          >
            <dl className="space-y-3">
              ...
              <div className="border-t pt-3 mt-3">
                ...
                <dd>
                  <RatingDisplay
                    rating={detail.stats.avgRating}
                    reviewCount={detail.stats.totalReviews}
                  />
                </dd>
              </div>
              <div className="flex items-center justify-between text-sm">
              ...
              <div className="flex items-center justify-between text-sm border-t pt-3">
                ...
                <dd className="font-semibold text-emerald-700">
                  {fmtCurrency(detail.stats.totalEarnings)}
                </dd>
              </div>
            </dl>
          </Card>
          ...
          >
            <dl className="space-y-3">
              <div className="flex items-center justify-between text-sm">
                ...
                <dd className="font-semibold text-[#111827]">
                  {fmtCurrency(detail.wallet.currentBalance)}
                </dd>
              </div>
              <div className="flex items-center justify-between text-sm">
                <dt className="text-[#6B7280]">Total Earned</dt>
                <dd className="font-semibold text-[#111827]">
                  {fmtCurrency(detail.wallet.totalEarned)}
                </dd>
              </div>
              <div className="flex items-center justify-between text-sm">
                <dt className="text-[#6B7280]">Total Withdrawn</dt>
                <dd className="font-semibold text-[#111827]">
                  {fmtCurrency(detail.wallet.totalWithdrawn)}
                </dd>
              </div>
              <div className="flex items-center justify-between text-sm">
                <dt className="text-[#6B7280]">Pending Payout</dt>
                <dd className="font-semibold text-[#111827]">
                  {fmtCurrency(detail.wallet.pendingPayout)}
                </dd>
              </div>
              <div className="flex items-center justify-between text-sm border-t pt-3">
                ...
                <dd>
                  <Badge
                    ...
                    className={cn(
                      "text-xs border",
                      PAYOUT_STATUS_STYLES[detail.wallet.payoutStatus],
                    )}
                  >
          ...
          <Card
            ...
            ariaLabel="Verification Documents"
          >
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        ...
                        <p className="text-xs text-[#6B7280]">
                          Submitted: {format(doc.submittedDate, "dd MMM yyyy")}
                        </p>
                      </div>
                      <Badge
                        variant="outline"
      ...
      <Card
        ...
        ariaLabel="Coach Media"
      >
        {detail.media.photos.length +
          detail.media.videos.length +
          detail.media.certificates.length >
        0 ? (
          <div>
            {/* Tabs */}
            <div className="flex gap-4 mb-4 border-b" role="tablist">
              {[
                {
                  key: "photos" as const,
                  label: `Photos (${detail.media.photos.length}/10)`,
                  icon: Image,
                },
                {
                  key: "videos" as const,
                  label: `Videos (${detail.media.videos.length}/3)`,
                  icon: Video,
                },
                {
                  key: "certificates" as const,
                  label: `Certificates (${detail.media.certificates.length}/5)`,
                  icon: FileText,
                },
              ].map((tab) => (
                <button
            ...
            <div role="tabpanel" className="grid grid-cols-4 gap-3">
              {mediaTab === "photos" &&
                (detail.media.photos.length > 0 ? (
                  detail.media.photos.map((p) => (
                    <div
                      key={p.id}
                  ...
                  <p className="col-span-4 text-sm text-[#9CA3AF]">
                    No photos uploaded.
                  </p>
                ))}
              {mediaTab === "videos" &&
                (detail.media.videos.length > 0 ? (
                  detail.media.videos.map((v) => (
                    <div
                      key={v.id}
                      className="aspect-video bg-gray-100 rounded-lg border border-gray-200 flex items-center justify-center col-span-2"
                    >
              ...
              {mediaTab === "certificates" &&
                (detail.media.certificates.length > 0 ? (
                  detail.media.certificates.map((cert) => (
                    <div
                      key={cert.id}
                      className="aspect-square bg-gray-100 rounded-lg border border-gray-200 flex items-center justify-center"
                    >
      ...
      {/* Recent Bookings */}
      <Card
        title={`Recent Bookings (${detail.recentBookings.length})`}
        icon={<CreditCard className="w-4 h-4" />}
        ...
        {detail.recentBookings.length > 0 ? (
                ...
                {detail.recentBookings.map((b) => (
                  <tr key={b.bookingId} className="hover:bg-gray-50/50">
                    ...
                    <td className="py-2 pr-4 text-[#6B7280] text-xs">
                      {format(b.bookingDate, "dd MMM yyyy, HH:mm")} UTC
                    </td>
                    <td className="py-2 pr-4 text-[#6B7280]">
                      {b.duration} min
                    </td>
          ...
          <div className="text-center py-8">
            ...
            <p className="text-xs text-[#6B7280] mt-1">
              This coach has not received any bookings.
            </p>
          </div>
        )}
      </Card>
      ...
      {/* Reviews Summary */}
      <Card
        title={`Reviews (${detail.stats.totalReviews})`}
        icon={<Star className="w-4 h-4" />}
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-2">
              {detail.ratingBreakdown.map((r) => {
                const pct =
                  detail.stats.totalReviews > 0
                    ? (r.count / detail.stats.totalReviews) * 100
                    : 0;
                return (
            ...
            <div className="space-y-4 border-t pt-4">
              <h4 className="text-xs text-[#6B7280] uppercase tracking-wide">
                Recent Reviews
              </h4>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-xs text-[#111827] ml-1 font-medium">
