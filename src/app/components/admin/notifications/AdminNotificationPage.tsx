...
export function AdminNotificationPage() {
  ...
  const [notifications, setNotifications] =
    useState<PushNotificationRecord[]>(MOCK_NOTIFICATIONS);
  ...
  const [viewTarget, setViewTarget] = useState<PushNotificationRecord | null>(
    null,
  );
  ...
  const estimate = useMemo(
    () => estimateRecipients(form.audiences),
    [form.audiences],
  );
  ...
  const validate = (): boolean => {
    ...
    if (!form.title.trim()) e.title = "Notification title is required.";
      ...
      e.title = `Title must not exceed ${TITLE_MAX} characters.`;
    if (!form.body.trim()) e.body = "Message body is required.";
    else if (form.body.length > BODY_MAX)
      e.body = `Message must not exceed ${BODY_MAX} characters.`;
    if (form.audiences.length === 0)
      e.audiences = "Please select at least one target audience.";
    if (form.deliveryType === "scheduled") {
      if (!form.scheduledAt)
        e.scheduledAt =
          "Please select a date and time for the scheduled notification.";
      else if (isPast(new Date(form.scheduledAt)))
        e.scheduledAt = "Scheduled date must be in the future.";
      else if (new Date(form.scheduledAt) > addDays(new Date(), 365))
        e.scheduledAt = "Scheduled date cannot exceed 365 days from today.";
    }
    setErrors(e);
  ...
  const handleEdit = (n: PushNotificationRecord) => {
    ...
    setForm({
      ...
      scheduledAt: n.scheduledAt
        ? new Date(n.scheduledAt).toISOString().slice(0, 16)
        : "",
    });
    setErrors({});
      ...
      prev.map((n) =>
        n.id === id
          ? {
              ...
              actualRecipients: Math.round(n.estimatedRecipients * 0.97),
              deliveryRate: 97,
            }
          : n,
      ),
    ...
    <div className="p-6 space-y-5 bg-[#F9FAFB] min-h-screen">
      ...
      <div className="grid grid-cols-[1fr_0.67fr] gap-6">
        ...
        <div
          className="space-y-5"
          role="form"
          aria-label="Compose push notification"
        >
          {/* Info Banner */}
          <div className="flex items-start gap-2.5 bg-blue-50 border border-blue-200 rounded-xl px-4 py-3">
            <Info className="h-4 w-4 text-blue-500 shrink-0 mt-0.5" />
            <p className="text-xs text-blue-700">
              Push notifications are delivered via push only and will not appear
              in users' in-app notification center.
            </p>
          </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5 space-y-4">
            ...
            <div>
              ...
              {errors.title && (
                <p
                  id="title-error"
                  className="flex items-center gap-1 text-[10px] text-red-600 mt-1"
                >
            ...
            {isScheduled && (
              <div>
                ...
                <Input
                  ...
                  aria-required="true"
                  aria-describedby={
                    errors.scheduledAt ? "schedule-error" : undefined
                  }
                />
                {errors.scheduledAt && (
                  <p
      ...
      <div className="bg-white rounded-xl border border-gray-200">
        ...
        <div className="px-5 py-3 border-b border-gray-100 flex items-center gap-3">
          <Select
            value={historyStatus}
            onValueChange={(v) => {
              setHistoryStatus(v as NotificationStatus | "All");
              setHistoryPage(1);
            }}
          >
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-gray-50/50">
                <TableHead className="text-[11px] uppercase tracking-wider text-gray-500">
                  Title
                </TableHead>
                <TableHead className="text-[11px] uppercase tracking-wider text-gray-500">
                  Audience
                ...
                <TableHead className="text-[11px] uppercase tracking-wider text-gray-500 w-28">
                  Actions
                </TableHead>
              </TableRow>
            </TableHeader>
                        ...
                        {n.status === "Scheduled" && n.scheduledAt ? (
                          <div>
                            ...
                            <p className="text-xs text-gray-700">
                              {format(parseISO(n.scheduledAt), "MMM d, HH:mm")}{" "}
                              UTC
                            </p>
                          </div>
                        ) : n.sentAt ? (
                          <div>
                            ...
                            <p className="text-xs text-gray-700">
                              {format(parseISO(n.sentAt), "MMM d, HH:mm")} UTC
                            </p>
                          </div>
                        ) : (
      ...
      <Dialog
        ...
        onOpenChange={(v) => {
          if (!v && !isSending) setConfirmSendOpen(false);
        }}
      >
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-3 py-2">
            <div className="bg-blue-50 border border-blue-100 rounded-xl p-3 text-center">
              ...
              <p className="text-3xl font-semibold text-[#003B95] tabular-nums">
                {estimate.toLocaleString()}
              </p>
            </div>
        ...
        <DialogContent className="max-w-xs">
          ...
          <DialogFooter>
            <Button
              variant="outline"
              size="sm"
              onClick={() => setClearFormOpen(false)}
            >
              Cancel
            </Button>
            <Button
              ...
              <div className="bg-gray-50 rounded-lg p-3 border border-gray-100 text-xs">
                ...
                {deleteTarget.scheduledAt && (
                  <p className="text-blue-600 flex items-center gap-1 mt-1">
                    ...
                    Scheduled for{" "}
                    {format(
                      parseISO(deleteTarget.scheduledAt),
                      "MMM d, yyyy 'at' HH:mm",
                    )}{" "}
                    UTC
                  </p>
                ...
                <div className="grid grid-cols-2 gap-3 pt-2 border-t border-gray-200">
                  ...
                  {viewTarget.scheduledAt && (
                    <div>
                      ...
                      <p className="text-gray-700">
                        {format(
                          parseISO(viewTarget.scheduledAt),
                          "MMM d, yyyy HH:mm",
                        )}{" "}
                        UTC
                      </p>
                  ...
                  {viewTarget.actualRecipients !== undefined && (
                    <div>
                      ...
                      <p className="text-gray-700">
                        {viewTarget.actualRecipients.toLocaleString()} /{" "}
                        {viewTarget.estimatedRecipients.toLocaleString()}
                      </p>
                    </div>
                  )}
...
function PushPreviewPanel({ title, body }: { title: string; body: string }) {
  ...
  const displayBody =
    body.trim() || "Your notification message will appear here...";
        ...
        <div className="relative w-[220px] h-[390px] rounded-[32px] bg-gray-900 shadow-2xl">
          ...
          <div className="absolute inset-[6px] rounded-[28px] overflow-hidden">
            ...
            <div className="absolute top-12 left-0 right-0 text-center">
              <p className="text-white/80 text-[10px] uppercase tracking-widest">
                Today
              </p>
            ...
            <div className="absolute left-2 right-2 top-[90px] rounded-2xl bg-white/25 backdrop-blur-md border border-white/20">
              <div className="flex items-center justify-between px-3 pt-2.5 pb-1">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-white/90 text-[9px] uppercase tracking-wide">
                    Playzoon
                  </span>
                </div>
              ...
              <div className="px-3 pb-3">
                ...
                <p className="text-[10px] leading-snug text-white/70">
                  {displayBody.length > 80
                    ? displayBody.slice(0, 80) + "..."
                    : displayBody}
                </p>
              </div>
              <div className="border-t border-white/15 flex">
                <span className="flex-1 py-1.5 text-[10px] text-[#60a5fa] text-center border-r border-white/15">
                  Open
                </span>
                <span className="flex-1 py-1.5 text-[10px] text-white/50 text-center">
                  Dismiss
                </span>
              </div>
            </div>
