...
import {
  Bell, BellRing, CheckCheck, Filter, ChevronRight, AlertTriangle,
  Building2, BadgeCheck, Banknote, HelpCircle, Flag, TrendingUp,
  StarOff, DoorOpen, Radio, UserCog, Trophy, Info, Wifi,
} from "lucide-react";
import { cn } from "../../../lib/utils";
import {
  AdminNotification, AdminNotifCategory, NOTIF_TYPE_CONFIG,
  CATEGORY_COLORS, ALL_CATEGORIES, POLL_INTERVAL_MS,
} from "./types";
...
function NotifRow({ notif, onRead }: { notif: AdminNotification; onRead: () => void }) {
  ...
  return (
    <button onClick={onRead}
      className={cn(
        ...
        notif.priority === "critical" && !notif.isRead && "bg-red-50/30 hover:bg-red-50/50"
      )}>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start gap-2 flex-wrap">
          <p className={cn("text-sm leading-snug flex-1", notif.isRead ? "text-gray-600" : "text-gray-900 font-semibold")}>
            {notif.title}
          </p>
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            <span className={cn("text-[10px] px-2 py-0.5 rounded-full border font-medium", catStyle)}>
              {notif.category}
            </span>
          </div>
        </div>
        ...
        {notif.entityName && (
          <p className="text-xs text-gray-400 mt-0.5">
            <span className="font-medium text-gray-600">{notif.entityName}</span>
            {notif.entityId && <span className="text-gray-300"> · {notif.entityId}</span>}
          </p>
        )}
      ...
      <div className="text-right shrink-0 min-w-[100px]">
        <p className="text-xs text-gray-700 tabular-nums">{format(notif.timestamp, "dd MMM yyyy")}</p>
        <p className="text-[10px] text-gray-400">{format(notif.timestamp, "HH:mm")}</p>
  ...
  return (
    <div className="p-6 lg:p-8 space-y-5 bg-[#F9FAFB] min-h-screen">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <div className="flex items-center gap-2 text-xs text-gray-400 mb-1.5">
            ...
            <span className="text-gray-600 font-medium">Notification Centre</span>
          </div>
          <h1 className="text-2xl font-bold tracking-tight text-gray-900 flex items-center gap-2.5">
            {unreadCount > 0 ? <BellRing className="w-6 h-6 text-[#003B95]" /> : <Bell className="w-6 h-6 text-[#003B95]" />}
            Notification Centre
          </h1>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: "Total",          value: stats.total,    color: "bg-white border-gray-200 text-gray-700",         icon: Bell         },
          { label: "Unread",         value: stats.unread,   color: "bg-blue-50 border-blue-200 text-blue-700",       icon: BellRing     },
          { label: "Critical Unread",value: stats.critical, color: stats.critical > 0 ? "bg-red-50 border-red-200 text-red-700" : "bg-gray-50 border-gray-200 text-gray-400", icon: AlertTriangle },
          { label: "Today",          value: stats.today,    color: "bg-emerald-50 border-emerald-200 text-emerald-700", icon: CheckCheck },
        ].map((s) => (
          <div key={s.label} className={cn("border rounded-xl p-4 flex items-center gap-3 shadow-sm", s.color)}>
      ...
      <div className="flex items-center gap-2 px-4 py-2.5 bg-white border border-gray-200 rounded-xl text-xs text-gray-500 shadow-sm">
        ...
        <span><strong>BR-130-05:</strong> Notification badge updates in real-time via polling every {POLL_INTERVAL_MS / 1000}s. New notifications appear automatically at the top.</span>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm flex flex-wrap gap-3 items-center">
          ...
          {(["All", "Unread", "Read"] as ReadFilter[]).map((f) => (
            <button key={f} onClick={() => { setReadFilter(f); setPage(1); }}
              ...
              {f === "Unread" && unreadCount > 0 && (
                <span className="ml-1.5 text-[10px] bg-blue-100 text-blue-700 font-bold px-1.5 py-0.5 rounded-full">{unreadCount}</span>
              )}
            </button>
          ))}
        </div>
        ...
        <span className="ml-auto text-xs text-gray-400">
          Showing {filtered.length} of {notifications.length} · Retained 90 days (BR-130-03)
        </span>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
        {grouped.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center px-6">
            ...
            <p className="text-sm text-gray-400 mt-1">No notifications match the selected filters.</p>
          </div>
          ...
          <div key={group}>
            <div className="px-5 py-2.5 bg-gray-50 border-b border-t border-gray-100 sticky top-0 z-10">
              ...
              <span className="ml-2 text-[10px] text-gray-400">{items.length} notification{items.length !== 1 ? "s" : ""}</span>
            </div>
            {items.map((notif) => (
              <NotifRow key={notif.id} notif={notif} onRead={() => handleClick(notif)} />
            ))}
          </div>
        ))}
          ...
          <div className="flex items-center justify-between px-5 py-3 border-t border-gray-100 bg-gray-50/50">
            ...
            <div className="flex gap-1">
              <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={safePage <= 1}
                className="px-3 py-1 border border-gray-300 rounded-md bg-white text-xs hover:bg-gray-50 disabled:opacity-40">Previous</button>
              <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={safePage >= totalPages}
                className="px-3 py-1 border border-gray-300 rounded-md bg-white text-xs hover:bg-gray-50 disabled:opacity-40">Next</button>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="space-y-2">
        <div className="flex items-start gap-2 px-4 py-2.5 bg-white border border-gray-200 rounded-xl text-xs text-gray-500">
          ...
          <span><strong>BR-130-01:</strong> Notifications are scoped to your Admin role and permissions. Sub-Admins only receive notifications within their assigned area.</span>
        </div>
        <div className="flex items-start gap-2 px-4 py-2.5 bg-red-50 border border-red-100 rounded-xl text-xs text-red-700">
          ...
          <span><strong>BR-130-04:</strong> System Error (Critical) notifications are always created in-app regardless of push/email notification settings.</span>
        </div>
      </div>
    </div>
  );
