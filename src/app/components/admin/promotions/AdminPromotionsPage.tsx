...
const PROMOTION_STATUS_CONFIG: Record<
  PromotionStatus,
...
function SortableHeader({
  ...
  return (
    <TableHead
      className={cn(
        "cursor-pointer select-none hover:bg-gray-100/50 transition-colors",
        className,
      )}
    >
      <button
...
export function AdminPromotionsPage() {
  ...
  const [providerTypeFilter, setProviderTypeFilter] = useState<
    ProviderType | "All"
  >("All");
  const [placementFilter, setPlacementFilter] = useState<PlacementType | "All">(
    "All",
  );
  ...
  const filtered = useMemo(() => {
    ...
    return MOCK_PROMOTIONS.filter((p) => {
      if (statusFilter !== "All" && p.promotionStatus !== statusFilter)
        return false;
      if (providerTypeFilter !== "All" && p.providerType !== providerTypeFilter)
        return false;
      if (placementFilter !== "All" && p.placement !== placementFilter)
        return false;
      if (q) {
        const match =
      ...
      <div className="bg-white rounded-xl border border-gray-200 px-4 py-3">
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <Select
            ...
            onValueChange={(v) => {
              setProviderTypeFilter(v as ProviderType | "All");
              setPage(1);
            }}
          >
            <SelectTrigger
          ...
          <Select
            ...
            onValueChange={(v) => {
              setPlacementFilter(v as PlacementType | "All");
              setPage(1);
            }}
          >
            <SelectTrigger
    ...
    <TooltipProvider>
      ...
      >
        ...
        <TableCell className="text-right">
          <span className="text-xs font-medium text-gray-900 whitespace-nowrap tabular-nums">
            {fmtSAR(promo.amount)}
          </span>
        </TableCell>
