...
export function AdminPaymentPage() {
  ...
  const updateTxn = useCallback(
    (id: string, updater: (t: PaymentTransaction) => PaymentTransaction) => {
  ...
  const handleRefund = useCallback(
    (data: {
      ...
      const newRecord = {
        ...
        status: (isWallet ? "Completed" : "Initiated") as
          | "Completed"
          | "Initiated"
          | "Failed",
      };
      ...
      if (isWallet) {
        toast.success(
          `AED ${data.amount.toFixed(2)} credited to player wallet immediately`,
          {
        ...
        toast.info(
          `Refund of AED ${data.amount.toFixed(2)} initiated with payment gateway`,
          {
  ...
  const handleMarkResolved = useCallback(
    (reason: string) => {
      ...
      toast.success("Transaction marked as resolved", {
        description:
          "BR-005: No financial action taken — recorded for audit trail",
        duration: 5000,
      });
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            View all platform transactions, process refunds, and resolve payment
            disputes. BR-003: read-only except refund and resolution actions.
          </p>
        </div>
