...
export function PaymentStats({ transactions }: PaymentStatsProps) {
  const stats = useMemo(() => {
    const successTxns = transactions.filter((t) =>
      ["Success", "Refunded", "Refund Initiated"].includes(t.status),
    );
    const failedTxns = transactions.filter((t) => t.status === "Failed");
    const pendingTxns = transactions.filter((t) => t.status === "Pending");
    const refundedTxns = transactions.filter((t) =>
      ["Refunded", "Refund Initiated"].includes(t.status),
    );
        ...
        return (
          ...
          >
            <div className="min-w-0">
              ...
              <p className="text-xl text-gray-900 mt-1 tabular-nums truncate">
                {c.value}
              </p>
              <p className="text-[11px] text-gray-400 mt-0.5">{c.sub}</p>
            </div>
            <div
              className={`flex items-center justify-center h-10 w-9 rounded-xl shrink-0 ${c.iconBg}`}
            >
