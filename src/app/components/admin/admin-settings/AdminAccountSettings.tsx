...
function getStrengthChecks(pw: string, current: string): StrengthCheck[] {
  ...
  return [
    ...
    {
      label: "Differs from current password (≥ 4 chars)",
      pass: diffCount >= 4 || pw !== current,
    },
  ];
}
...
export function AdminAccountSettings() {
  ...
  const saveProfile = () => {
    ...
    if (displayName.length > 60) {
      toast.error("Display name cannot exceed 60 characters.");
      return;
    }
    setProfile((p) => ({ ...p, displayName: displayName.trim() }));
    toast.success(
      "Profile updated. Your display name will appear in audit logs and sub-admin views.",
    );
  };
  ...
  const handlePasswordChange = async () => {
    if (pwLocked) {
      toast.error(
        "Password change is locked for 30 minutes due to too many failed attempts.",
      );
      return;
    }
    ...
    try {
      ...
      setConfirmPw("");
      toast.success(
        "Password changed successfully. A security notification has been sent to your email.",
      );
    } catch (error: any) {
      ...
      if (code === "UNAUTHORIZED" || msg.toLowerCase().includes("incorrect")) {
        ...
        if (attempts >= 3) {
          setPwLocked(true);
          toast.error(
            "Too many incorrect attempts. Password change locked for 30 minutes (BR-127-05).",
          );
        } else {
          toast.error(
            `Incorrect current password. ${3 - attempts} attempt${3 - attempts !== 1 ? "s" : ""} remaining.`,
          );
        }
      } else {
        toast.error(msg);
  ...
  const verifyOtp = async () => {
    ...
    if (otp !== MOCK_OTP) {
      ...
      if (attempts >= 3) {
        toast.error(
          "Maximum OTP attempts reached. Please restart the email change process.",
        );
        setOtpSent(false);
        setNewEmail("");
        setOtp("");
  ...
  // ── 2FA (BR-127-03) ──────────────────────────────────────
  const [twoFaMethod, setTwoFaMethod] = useState<"totp" | "sms">(
    profile.twoFactorMethod,
  );
  const [pendingMethod, setPendingMethod] = useState<"totp" | "sms" | null>(
    null,
  );
  ...
  const confirmTwoFaSwitch = async () => {
    if (verifyCode.length < 6) {
      toast.error("Enter your current 6-digit verification code.");
      return;
    }
    ...
    setVerifyCode("");
    toast.success(
      `2FA method switched to ${pendingMethod === "totp" ? "Authenticator App" : "SMS"}. Please set up your new method.`,
    );
  };
  ...
  const logoutAllDevices = async () => {
    ...
    toast.success(
      "All other sessions terminated (BR-127-06). Your current session remains active.",
    );
  };
    ...
    <div className="p-6 space-y-5 bg-[#F9FAFB] min-h-screen">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-0.5">
          Manage your profile, security credentials, and active sessions.
        </p>
      </div>
      ...
      <Section
        icon={Upload}
        title="Admin Profile"
        desc="Display name and photo shown in audit logs and sub-admin views."
          ...
          <div className="flex-1 space-y-3">
            <div className="space-y-1.5">
              ...
              <p
                className={cn(
                  "text-[10px]",
                  displayName.length > 50 ? "text-amber-500" : "text-gray-400",
                )}
              >
            ...
            <div className="space-y-1.5">
              ...
              <p className="text-xs text-gray-400">
                To change email, use the Email Address section below.
              </p>
            </div>
            <Button
              onClick={saveProfile}
      ...
      <Section
        ...
        desc="Minimum 10 characters with uppercase, lowercase, number, and special character."
        color="text-blue-600 bg-blue-50"
      >
        {pwLocked && (
          <div className="flex items-start gap-2.5 p-3 bg-red-50 border border-red-200 rounded-xl mb-4 text-sm text-red-700">
            ...
            <div>
              ...
              <p className="text-xs mt-0.5">
                Too many failed attempts. Password change is locked for 30
                minutes (BR-127-05).
              </p>
            </div>
          </div>
          ...
          {!otpSent ? (
            <>
              ...
              <div className="flex items-start gap-2 text-xs text-blue-700 bg-blue-50 border border-blue-100 rounded-lg px-3 py-2">
                ...
                An OTP will be sent to the new email. The change is not applied
            ...
            <>
              <div className="flex items-start gap-2 text-xs text-emerald-700 bg-emerald-50 border border-emerald-100 rounded-lg px-3 py-2">
                ...
                OTP sent to <strong>{newEmail}</strong>. It expires in 10
                minutes. (Mock OTP: <strong>123456</strong>)
              </div>
      ...
      >
        <div className="space-y-4">
          ...
          <div className="flex items-start gap-2.5 p-3 bg-red-50 border border-red-200 rounded-xl text-sm text-red-800">
            ...
            <div>
              <p className="font-semibold">
                Two-Factor Authentication is mandatory for Admin accounts and
                cannot be disabled.
              </p>
              <p className="text-xs mt-0.5 text-red-700">
                You may switch between methods by verifying your current 2FA
                code first.
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {/* TOTP */}
            <button
              onClick={() => requestTwoFaSwitch("totp")}
              className={cn(
            ...
            {/* SMS */}
            <button
              onClick={() => requestTwoFaSwitch("sms")}
              className={cn(
                "flex items-start gap-3 p-4 rounded-xl border-2 text-left transition-all",
                twoFaMethod === "sms"
                  ? "border-blue-600 bg-blue-50/50 ring-1 ring-blue-200"
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-500 mt-0.5">
                  Receive a 6-digit code via SMS to {profile.smsPhone}.
                </p>
              </div>
            </button>
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            <span className="inline-flex items-center gap-1.5 text-xs text-blue-700 bg-blue-50 border border-blue-100 px-2.5 py-1 rounded-full">
              <Info className="w-3 h-3" /> BR-127-07: Cannot terminate current
              session from this view
            </span>
            <span className="inline-flex items-center gap-1.5 text-xs text-emerald-700 bg-emerald-50 border border-emerald-100 px-2.5 py-1 rounded-full">
              <CheckCircle2 className="w-3 h-3" /> BR-127-06: Log Out All
              Devices keeps current session active
            </span>
          </div>
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="text-left px-4 py-2.5 text-xs font-medium text-gray-500 uppercase tracking-wide hidden lg:table-cell">
                    Location
                  </th>
                  <th className="text-left px-4 py-2.5 text-xs font-medium text-gray-500 uppercase tracking-wide">
                    Last Active
                ...
                {sessions.map((session) => (
                  <tr
                    ...
                    className={cn(
                      ...
                      session.isCurrent && "bg-blue-50/30",
                    )}
                  >
                    {/* Device */}
            ...
            <div className="flex items-center justify-between p-3 bg-red-50/50 border border-red-100 rounded-xl">
              <div>
                <p className="text-sm font-medium text-red-800">
                  {otherSessionsCount} other active session
                  {otherSessionsCount !== 1 ? "s" : ""}
                </p>
                <p className="text-xs text-red-600 mt-0.5">
                  Terminate all sessions except this one. Your current session
                  remains active.
                </p>
              </div>
              <Button
            ...
            <AlertDialogDescription asChild>
              <div className="space-y-4 mt-1">
                <p className="text-sm text-gray-600">
                  ...
                  , enter your current{" "}
                  <strong>
                    {twoFaMethod === "totp" ? "TOTP code" : "SMS code"}
                  </strong>{" "}
                  to authorise the change (BR-127-03).
                </p>
                <div className="space-y-2">
                  <Label>
                    Current {twoFaMethod === "totp" ? "Authenticator" : "SMS"}{" "}
                    Code
                  </Label>
                  <OTPInput value={verifyCode} onChange={setVerifyCode} />
                  <p className="text-[10px] text-gray-400">
                    Enter any 6-digit code to proceed in mock mode.
                  </p>
                </div>
              </div>
            </AlertDialogDescription>
      ...
      >
        <AlertDialogContent className="z-[60]">
          <AlertDialogHeader>
            <AlertDialogTitle className="flex items-center gap-2">
              <LogOut className="w-5 h-5 text-red-600" /> Log Out All Other
              Devices?
            </AlertDialogTitle>
            <AlertDialogDescription asChild>
